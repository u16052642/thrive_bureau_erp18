...
from thrive import api, fields, models, modules, tools
...
from thrive.tools import _, LazyTranslate
...
class AccountOnlineAccount(models.Model):
    ...
    def _assign_journal(self, swift_code=False):
        ...
        Also, Create and assign bank & swift/bic code if thrivefin returns one
...
class AccountOnlineLink(models.Model):
    ...
    @api.model
    def _get_thrivefin_url(self, url):
    ...
    def _fetch_thrive_fin(self, url, data=None, ignore_status=False):
        ...
        endpoint_url = self._get_thrivefin_url(url)
            ...
            if reset_tx:
                ...
                if subject and message:
                    ...
                    if context.get('redirect_warning_url'):
                        if context['redirect_warning_url'] == 'thrive_support':
                            ...
                            message_post = Markup('%s<br>%s <a href="%s" >%s</a>') % (message, _("You can contact Thrive support"), url, _("Here"))
                            button_label = _('Report issue')
                        else:
                            url = "https://www.thrivebureau.com/documentation/17.0/applications/finance/accounting/bank/bank_synchronization.html#faq"
                ...
                self.env.cr.commit()
                # and then raise either a redirectWarning error so that customer can easily open an issue with Thrive,
        ...
        for link in self:
            try:
                ...
                if resp_json.get('delete', True) is True:
                    to_unlink += link
            except ThriveFinRedirectException:
