...
class AccountJournal(models.Model):
    ...
    def fetch_online_sync_favorite_institutions(self):
        ...
        endpoint_url = self.env['account.online.link']._get_thrivefin_url('/proxy/v1/get_dashboard_institutions')
        ...
        try:
            ...
            for institution in resp_dict:
                if institution['picture'].startswith('/'):
                    institution['picture'] = self.env['account.online.link']._get_thrivefin_url(institution['picture'])
            return resp_dict
