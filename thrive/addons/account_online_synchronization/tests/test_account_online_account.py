...
class TestAccountOnlineAccount(AccountOnlineSynchronizationCommon):
    ...
    @patch('thrive.addons.account_online_synchronization.models.account_online.AccountOnlineAccount._refresh', return_value={'success': True, 'data': {}})
    ...
    def test_delete_with_redirect_error(self, patched_request):
        ...
        # such error are returned as a ThriveFinRedirectException with mode link to reopen the iframe and link with a new
        ...
        first_call = self._mock_thrivefin_error_response(code=102)
        second_call = self._mock_thrivefin_error_response(code=300, data={'mode': 'link'})
    ...
    def test_redirect_mode_link(self, patched_request):
        # Use case being tested: Call to open the iframe which result in a ThrivefinRedirectException in link mode
        ...
        first_call = self._mock_thrivefin_error_response(code=300, data={'mode': 'link'})
        second_call = self._mock_thrivefin_response(data={'delete': True})
        patched_request.post.side_effect = [first_call, second_call]
