...
import { click, queryAll, queryFirst } from "@thrive/hoot-dom";
...
test("initial action crashes", async () => {
    expect.errors(1);
    redirect("/thrive/action-__test__client__action__?menu_id=1");
...
test("Navigate to an application from the HomeMenu should generate only one pushState", async () => {
    ...
    await animationFrame();
    expect(".o_home_menu").toHaveCount(1);
    expect.verifySteps(["/thrive", "/thrive/action-1002", "/thrive", "/thrive/action-1001", "/thrive"]);
});
