...
import { click, queryFirst, edit } from "@thrive/hoot-dom";
import { animationFrame, mockDate, runAllTimers } from "@thrive/hoot-mock";
import {
    getService,
    mockService,
    mountWithCleanup,
...
test("Expiration Panel one app installed, buy subscription", async () => {
    ...
    expect(browser.location.href).toBe("https://www.thrivebureau.com/thrive-enterprise/upgrade?num_users=7");
});
...
test("One app installed, navigation to renewal page", async () => {
    ...
    expect(browser.location.href).toBe("https://www.thrivebureau.com/thrive-enterprise/renew?contract=ABC");
