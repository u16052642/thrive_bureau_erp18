...
import thrive.http
from thrive.tools.misc import hmac, DotDict, frozendict
...
def MockRequest(
    ...
    from thrive.tests.common import HttpCase  # noqa: PLC0415
    ...
    if HttpCase.http_port():
        ...
        base_url = f"http://{HOST}:{thrive.tools.config['http_port']}"
    request = Mock(
        # request
        httprequest=Mock(
            host='localhost',
    ...
    if country_code:
        request.geoip._city_record = thrive.http.geoip2.models.City({'country': {'iso_code': country_code}})
