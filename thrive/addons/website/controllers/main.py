...
from thrive.tools import OrderedSet, escape_psql, html_escape as escape, py_to_js_locale
from thrive.addons.base.models.ir_http import EXTENSION_TO_WEB_MIMETYPES
...
from thrive.tools.json import scriptsafe as json
...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/thrive?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/thrive/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(f"/thrive/ir.ui.view/{page.get('view_id')}")
