...
export class WebsitePreview extends Component {
        ...
        onWillStart(async () => {
            ...
            if (this.websiteDomain && !wUtils.isHTTPSorNakedDomainRedirection(this.websiteDomain, window.location.origin)) {
                ...
                this.dialogService.add(WebsiteDialog, {
                    ...
                    onClose: () => {
                        redirect(`${encodeURI(this.websiteDomain)}/thrive/action-website.website_preview?path=${encodedPath}&website_id=${encodeURIComponent(this.websiteId)}`);
                    }
                });
            } else {
    ...
    _isTopWindowURL({ host, pathname }) {
        const backendRoutes = ['/web', '/web/session/logout', '/thrive'];
    ...
    /**
     * This replaces the browser url (/thrive/action-website...) with
    ...
    _replaceBrowserUrl() {
        if (!wUtils.isHTTPSorNakedDomainRedirection(this.iframe.el.contentWindow.location.origin, window.location.origin)) {
            ...
            // trigger a traceback.
            history.replaceState(history.state, document.title, '/thrive');
            return;
        }
    ...
    _onKeydownRefresh(ev) {
        ...
        redirect(`/thrive/action-website.website_preview?path=${encodeURIComponent(path)}${debugMode}`);
    }
}
