...
from thrive.addons.mail.tools.jwt import InvalidVapidError
...
class TestWebPushNotification(SMSCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env['mail.push.device'].sudo().create([
            {
                'endpoint': f'https://test.thrivebureau.com/webpush/user{(idx + 1)}',
    ...
    @mute_logger('thrive.tests')
    def test_notify_by_push(self, push_to_end_point):
        """ When posting a comment, notify both inbox and people outside of Thrive
        aka email """
    ...
    def test_notify_by_push_channel_with_channel_notifications_settings(self, push_to_end_point):
        ...
        self.env["mail.push.device"].sudo().create(
            [
                {
                    "endpoint": f"https://test.thrivebureau.com/webpush/user{(idx + 20)}",
                    "expiration_time": None,
                    "keys": json.dumps(
                        {
                            "p256dh": "BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A",
        ...
        self.assertEqual(push_to_end_point.call_args.kwargs["device"]["endpoint"], "https://test.thrivebureau.com/webpush/user20")
        push_to_end_point.reset_mock()
        ...
        self.assertEqual(push_to_end_point.call_args_list[0].kwargs["device"]["endpoint"], "https://test.thrivebureau.com/webpush/user20")
        self.assertEqual(push_to_end_point.call_args_list[1].kwargs["device"]["endpoint"], "https://test.thrivebureau.com/webpush/user21")
        push_to_end_point.reset_mock()
    ...
    @patch.object(thrive.addons.mail.models.mail_push, 'push_to_end_point')
    ...
    def test_push_notifications_error_device_unreachable(self, post):
        with mute_logger('thrive.addons.mail.tools.web_push'):
            self.record_simple.with_user(self.user_email).message_notify(
                partner_ids=self.user_inbox.partner_id.ids,
                body='Test message send via Web Push',
                subject='Test Activity',
        ...
        notification_count = self.env['mail.push.device'].search_count([('endpoint', '=', 'https://test.thrivebureau.com/webpush/user2')])
