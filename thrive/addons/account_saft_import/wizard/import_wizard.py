...
class SaftImportWizard(models.TransientModel):
    ...
    def _get_account_types(self):
        """ Returns a mapping between the account types accepted for the SAF-T and the types in Thrive """
        # To be overriden
        return {}
    ...
    def _prepare_account_data(self, tree):
        ...
        :returns: account_mapping_ids: mapping between ids coming from the SAF-T with the ones from Thrive Bureau ERP and the balance
        """
    ...
    def _prepare_opening_balance_move(self, tree, map_accounts):
        """ Create a move if there is inconsistency between opening balance for each account and amls in Thrive
    ...
    def _prepare_partner_data(self, tree):
        ...
        :returns: partner_mapping_ids: mapping between ids coming from the SAF-T with the ones from Thrive Bureau ERP
        """
        nsmap = self._get_cleaned_namespace(tree)
        partners_to_create = {}
    ...
    def _prepare_tax_data(self, tree):
        ...
        :returns: tax_mapping_ids: mapping between ids coming from the SAF-T with the ones used in Thrive
        """
        nsmap = self._get_cleaned_namespace(tree)
    ...
    def _prepare_journal_data(self, tree, default_currency, map_accounts, map_taxes, map_currencies, map_partners):
        ...
        :param map_accounts: mapping between saft and thrive ids (and balance) for accounts
        :param map_taxes: mapping between saft and thrive ids for taxes
        :param map_currencies: mapping between saft and thrive ids for taxes
        :param map_partners: mapping between saft and thrive ids for partners
    ...
    def _prepare_move_data(self, journal_tree, default_currency, saft_journal_code, journal_id, map_accounts, map_taxes, map_currencies, map_partners):
        ...
        :param journal_id: thrive id of the journal
        :param map_accounts: mapping between saft and thrive ids (and balance) for accounts
        :param map_taxes: mapping between saft and thrive ids for taxes
        :param map_currencies: mapping between saft and thrive ids for taxes
        :param map_partners: mapping between saft and thrive ids for partners
