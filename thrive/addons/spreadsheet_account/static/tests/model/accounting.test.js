...
test("evaluation with reference to a month period", async () => {
    ...
    setCellContent(model, "A1", `=THRIVE.CREDIT("100", B1)`);
    setCellContent(model, "A2", `=THRIVE.DEBIT("100", B1)`);
    setCellContent(model, "A3", `=THRIVE.BALANCE("100", B1)`);
    await waitForDataLoaded(model);
    expect(getCellValue(model, "A1")).toBe(16);
    expect(getCellValue(model, "A2")).toBe(42);
    expect(getCellValue(model, "A3")).toBe(26);
...
test("date with non-standard locale", async () => {
    ...
    setCellContent(model, "A2", "=THRIVE.BALANCE(100, A1)");
    setCellContent(model, "A3", "=THRIVE.CREDIT(100, A1)");
    setCellContent(model, "A4", "=THRIVE.DEBIT(100, A1)");
    await waitForDataLoaded(model);
