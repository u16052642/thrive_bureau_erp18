...
import { queryFirst } from "@thrive/hoot-dom";
import { animationFrame, mockDate } from "@thrive/hoot-mock";
...
test("Create a new relational global filter with a chart", async function () {
    ...
    expect(globalFilter.label).toBe("Product");
    expect(globalFilter.defaultValue).toEqual([]);
    expect(model.getters.getThriveChartFieldMatching(chartId, globalFilter.id)).toEqual({
        chain: "product_id",
        type: "many2one",
    });
});
...
test("Create a new date filter", async function () {
    ...
    expect(model.getters.getThriveChartFieldMatching(chartId, globalFilter.id).offset).toBe(0);
});
...
test("Create a new date filter with period offsets", async function () {
    ...
    expect(model.getters.getThriveChartFieldMatching(chartId, globalFilter.id).offset).toBe(-2);
