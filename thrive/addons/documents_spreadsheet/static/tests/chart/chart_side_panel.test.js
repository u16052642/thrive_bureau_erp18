...
import { expect, test, beforeEach, getFixture, describe } from "@thrive/hoot";
...
test("From an Thrive chart, can only change to an Thrive chart", async () => {
    const { model, env } = await createSpreadsheetFromGraphView();
    await openChartSidePanel(model, env);
    const target = getFixture();
    await contains(".o-type-selector").click();
    const thriveChartTypes = chartSubtypeRegistry
        .getKeys()
        .filter((key) => key.startsWith("thrive_"))
        .sort();
    ...
    const optionValues = Array.from(options)
        ...
        .sort();
    expect(optionValues).toEqual(thriveChartTypes);
});
...
test("From a spreadsheet chart, can only change to a spreadsheet chart", async () => {
    ...
    const optionValues = Array.from(options)
        .map((option) => option.dataset.id)
        .sort();
    const nonThriveChartTypes = chartSubtypeRegistry
        .getKeys()
        .filter((key) => !key.startsWith("thrive_"))
        .sort();

    expect(optionValues).toEqual(nonThriveChartTypes);
});

test("Possible chart types are correct when switching from a spreadsheet to an thrive chart", async () => {
    const { model, env } = await createSpreadsheetFromGraphView();
    createBasicChart(model, "nonThriveChartId");
    await openChartSidePanel(model, env);
    const target = getFixture();
    await contains(".o-type-selector").click();
    ...
    expect(optionValues.every((value) => value.startsWith("thrive_"))).toBe(true);

    model.dispatch("SELECT_FIGURE", { id: "nonThriveChartId" });
...
test("Change thrive chart type", async () => {
    const { model, env } = await createSpreadsheetFromGraphView();
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    expect(model.getters.getChart(chartId).type).toBe("thrive_bar");
    await openChartSidePanel(model, env);
    /** @type {HTMLSelectElement} */
    await changeChartType("thrive_pie");
    expect(model.getters.getChart(chartId).type).toBe("thrive_pie");

    await changeChartType("thrive_line");
    ...
    await changeChartType("thrive_bar");
    expect(model.getters.getChart(chartId).type).toBe("thrive_bar");
    expect(model.getters.getChart(chartId).stacked).toBe(false);

    await changeChartType("thrive_stacked_bar");
    expect(model.getters.getChart(chartId).type).toBe("thrive_bar");
    expect(model.getters.getChart(chartId).stacked).toBe(true);

    await changeChartType("thrive_stacked_line");
    expect(model.getters.getChart(chartId).type).toBe("thrive_line");
    expect(model.getters.getChart(chartId).stacked).toBe(true);
});
...
test("Thrive area chart", async () => {
    const { model, env } = await createSpreadsheetFromGraphView();
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("thrive_area");

    let chartDefinition = model.getters.getChartDefinition(chartId);
    expect(chartDefinition.type).toBe("thrive_line");
    expect(chartDefinition.fillArea).toBe(true);
    expect(chartDefinition.stacked).toBe(false);

    await changeChartType("thrive_stacked_area");
...
test("Open chart thrive's data properties", async function () {
    const target = getFixture();
    const { model, env } = await createSpreadsheetFromGraphView();
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
