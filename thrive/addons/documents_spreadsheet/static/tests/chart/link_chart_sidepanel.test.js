...
import { beforeEach, describe, expect, getFixture, test } from "@thrive/hoot";
...
import { manuallyDispatchProgrammaticEvent } from "@thrive/hoot-dom";
...
test("can link an thrive menu to a basic chart chart in the side panel", async function () {
    ...
    expect(thriveMenu).toBe(undefined, { message: "No menu linked with chart at start" });

    expect(".o-ir-menu-selector input").toHaveCount(1, {
        message: "A menu to link charts to thrive menus was added to the side panel",
    });
    ...
    expect(thriveMenu.xmlid).toBe("documents_spreadsheet.test.menu", {
        message: "Thrive menu is linked to chart",
    });
});

test("can link an thrive menu to a scorecard chart chart in the side panel", async function () {
    const { model } = await createSpreadsheet({ serverData });
...
test("can link an thrive menu to a gauge chart chart in the side panel", async function () {
    const { model } = await createSpreadsheet({ serverData });
...
test("can remove link between an thrive menu and a chart in the side panel", async function () {
    const { model } = await createSpreadsheet({ serverData });
    createBasicChart(model, chartId);
    await animationFrame();
    ...
    expect(thriveMenu).toBe(undefined, { message: "no menu is linked to chart" });
});
