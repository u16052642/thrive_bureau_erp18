    ...
    const spreadsheetData = {
        ...
        sheets: [
            ...
            },
            {
                id: "sh2",
                cells: {
                    A2: { content: `=THRIVE.PIVOT(1,"probability","bar","false","foo",2)` },
                },
            },
        ],
        pivots: {
...
test("Can Insert thrive formulas from Insert > Functions > Thrive", async function () {
    const { model } = await createSpreadsheet();

    setCellContent(model, "A1", `Hi :)`);

    // Skip the visibility checks because on small screens (height <~700 px) the menus become too
    // tall for the screen and need to be scrolled to have the "Thrive" menu item visible.
    ...
    await contains(".o-menu-item[title='Thrive']", skipVisibilityChecks).click();
    await contains(".o-menu-item[title='THRIVE.CURRENCY.RATE']", skipVisibilityChecks).click();

    await press("Enter");
    await animationFrame();

    expect(getCellContent(model, "A1")).toBe("=THRIVE.CURRENCY.RATE()");
});
