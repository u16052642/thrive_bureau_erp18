...
export class SpreadsheetAction extends AbstractSpreadsheetAction {
    ...
    async saveAsTemplate() {
        const model = new Model(this.model.exportData(), {
            custom: {
                env: this.env,
                thriveDataProvider: this.model.config.custom.thriveDataProvider,
            },
        });
    ...
    async freezeAndShareSpreadsheet() {
        ...
        const { freezeThriveData } = thrive.loader.modules.get("@spreadsheet/helpers/model");
        const data = await freezeThriveData(this.model);
