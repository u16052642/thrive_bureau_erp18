...
class Document(models.Model):
    ...
    def _contains_live_data(self):
        """Return true if the spreadsheet contains live data, like Thrive pivots, chart, etc."""
        self.ensure_one()
        if self.handler != 'spreadsheet':
            return False

        snapshot = self._get_spreadsheet_snapshot()
        if snapshot.get("lists") or snapshot.get("pivots") or snapshot.get("chartThriveMenusReferences"):
            return True

        revisions = self._build_spreadsheet_messages()
        return any(  # modification not yet committed in snapshot
            command.get("type")
            in ("INSERT_THRIVE_LIST", "ADD_PIVOT", "LINK_THRIVE_MENU_TO_CHART")
