    ...
    def _write_from_google(self, gevent, vals):
        ...
        for thrive_attendee_email in set(existing_attendees.mapped('email')):
            ...
            if email_normalize(thrive_attendee_email) not in emails:
                attendees = existing_attendees.exists().filtered(lambda att: att.email == email_normalize(thrive_attendee_email) and att.partner_id not in organizers_partner_ids)
