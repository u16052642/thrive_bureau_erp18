...
from thrive.addons.google_account.models.google_service import _get_client_secret
...
class User(models.Model):
    ...
    def _check_pending_thrive_records(self):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        if not results or (not results.get('events') and not self._check_pending_thrive_records()):
            return False
        ...
        # We apply Google updates only if their write date is later than the write date in Thrive.
        # It's possible that multiple updates affect the same record, maybe not directly.
        # To handle this, we preserve the write dates in Thrive before applying any updates,
        # and use these dates instead of the current live dates.
        thrive_events = self.env['calendar.event'].browse((events - recurrences).thrive_ids(self.env))
        thrive_recurrences = self.env['calendar.recurrence'].browse(recurrences.thrive_ids(self.env))
        recurrences_write_dates = {r.id: r.write_date for r in thrive_recurrences}
        events_write_dates = {e.id: e.write_date for e in thrive_events}
        synced_recurrences = self.env['calendar.recurrence']._sync_google2thrive(recurrences, recurrences_write_dates)
        synced_events = self.env['calendar.event']._sync_google2thrive(events - recurrences, events_write_dates, default_reminders=default_reminders)
