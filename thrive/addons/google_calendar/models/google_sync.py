...
class GoogleSync(models.AbstractModel):
    ...
    @api.model
    def _sync_google2thrive(self, google_events: GoogleEvent, write_dates=None, default_reminders=()):
        ...
        :param write_dates: A dictionary mapping Thrive record IDs to their write dates.
        ...
        for gevent in pending:
            ...
            # Use the record's write_date to apply Google updates only if they are newer than Thrive's write_date.
            thrive_record_write_date = write_dates.get(thrive_record.id, thrive_record.write_date)
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not thrive_record_write_date or updated >= pytz.utc.localize(thrive_record_write_date):
    ...
    def _check_any_records_to_sync(self):
        """ Returns True if there are pending records to be synchronized from Thrive Bureau ERP to Google, False otherwise. """
        is_active_clause = (self._active_name, '=', True) if self._active_name else expression.TRUE_LEAF
