...
from thrive.addons.google_calendar.utils.google_calendar import GoogleEvent, GoogleCalendarService
...
class TestSyncGoogle2Thrive(TestSyncGoogle):
    ...
    def test_event_with_local_videocall(self):
        ...
        event = self.env['calendar.event'].create({
            ...
            'videocall_location': 'https://meet.google.com/thrive_local_videocall',
        })
        values = {
            'id': google_id,
            'status': 'confirmed',
        ...
        self.env['calendar.event']._sync_google2thrive(gevent)
        self.assertEqual(event.videocall_location, 'https://meet.google.com/thrive_local_videocall')

        # now google has meet URL and make sure local video call is updated accordingly
        values['conferenceData']['entryPoints'][0]['uri'] = 'https://meet.google.com/thrive-random-test'
        gevent = GoogleEvent([values])
        self.env['calendar.event']._sync_google2thrive(gevent)
        self.assertEqual(event.videocall_location, 'https://meet.google.com/thrive-random-test')
        self.assertGoogleAPINotCalled()
    ...
    def test_create_event_with_default_and_undefined_privacy(self):
        """ Check if google events are created in Thrive when 'default' privacy setting is defined and also when it is not. """
        # Sync events from Google to Thrive Bureau ERP after adding the privacy property.
        sample_event_values = {
            'summary': 'Test',
        ...
        self.env['calendar.event']._sync_google2thrive(GoogleEvent([undefined_privacy_event, default_privacy_event]))

        # Ensure that synced events have the correct privacy field in Thrive.
        undefined_privacy_thrive_event = self.env['calendar.event'].search([('google_id', '=', 1)])
        default_privacy_thrive_event = self.env['calendar.event'].search([('google_id', '=', 2)])
        ...
        self.assertFalse(default_privacy_thrive_event.privacy, "Event with default privacy must have False value in privacy field.")
    ...
    def test_accepting_recurrent_event_with_this_event_option_synced_by_attendee(self, mock_get_events):
        ...
        Ensure that event is accepeted by attendee in Thrive.
        """
        recurrence_id = "abcd1"
    ...
    def test_accepting_recurrent_event_with_all_events_option_synced_by_attendee(self, mock_get_events):
        ...
        Ensure that all events are accepeted by attendee in Thrive.
        """
        recurrence_id = "abcd3"
        recurrence = self.generate_recurring_event(
            mock_dt="2024-04-20",
    ...
    def test_accepting_recurrent_event_with_following_events_option_synced_by_attendee(self, mock_get_events):
        ...
        Ensure that affected events are accepeted by attendee in Thrive.
        """
        recurrence_id = "abcd5"
        recurrence = self.generate_recurring_event(
            mock_dt="2024-04-20",
    ...
    def test_keep_organizer_attendee_writing_recurrence_from_google(self):
        ...
        self.env['calendar.recurrence'].with_user(self.organizer_user)._sync_google2thrive(GoogleEvent(values))
        events = recurrence.calendar_event_ids.sorted('start')
