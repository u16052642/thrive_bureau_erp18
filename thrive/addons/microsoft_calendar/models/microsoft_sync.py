...
class MicrosoftSync(models.AbstractModel):
    ...
    def _sync_microsoft2thrive(self, microsoft_events: MicrosoftEvent):
        ...
        # Get sync lower bound days range for checking if old events must be updated in Thrive.
        ...
        for mevent in (existing - cancelled).filter(lambda e: e.lastModifiedDateTime):
            ...
            if thrive_event:
                ...
                if lower_bound_day_range:
                    update_time_diff = ms_event_updated_time - thrive_event_updated_time
                    old_event_update_condition = thrive_event._check_old_event_update_required(int(lower_bound_day_range), update_time_diff)

                if ms_event_updated_time >= thrive_event_updated_time and old_event_update_condition:
    ...
    def _check_old_event_update_required(self, lower_bound_day_range, update_time_diff):
        """
        Checks if an old event in Thrive should be updated locally. This verification is necessary because
        sometimes events in Thrive have the same state in Microsoft and even so they trigger updates locally
    ...
    def _microsoft_attendee_answer(self, answer, params, timeout=TIMEOUT):
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                # former differs for each attendee. This info is required for sending the event answer and Thrive currently
