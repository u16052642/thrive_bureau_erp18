...
class TestUpdateEvents(TestCommon):
    ...
    def test_restart_sync_with_synced_recurrence(self, mock_patch):
        """ Ensure that sync restart is not blocked when there are recurrence outliers in Thrive database. """
    ...
    def test_update_old_event_synced_with_outlook(self, mock_get_events, mock_write_from_microsoft):
        """
        There are old events in Thrive which share the same state with Microsoft and get updated (without changes) in Thrive
        due to a few seconds of update time difference, triggering lots of unwanted spam for attendees on Microsoft side.
        Don't update old events in Thrive if update time difference between Microsoft and Thrive is not significant.
        """
        ...
        # Mock the modification time in Microsoft with 10 minutes ahead Thrive event 'write_date'.
        # Synchronize Microsoft Calendar and ensure that the skipped event was not updated in Thrive.
        mock_get_events.return_value = (
            MicrosoftEvent([dict(
                self.simple_event_from_outlook_organizer,
