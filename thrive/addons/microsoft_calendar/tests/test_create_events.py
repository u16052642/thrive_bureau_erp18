...
from thrive.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token, _modified_date_in_the_future
...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_from_outlook_attendee_calendar_where_email_addresses_are_capitalized(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Thrive attendee calendar.
        The email addresses of the attendee and the organizer are in different case than in Thrive.
        """
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        ...
        # In Thrive, this event will be created (behind the screens) by a synced Thrive user as attendee (self.attendee_user).
        ...
        # Mock event from Microsoft and sync event with Thrive through self.attendee_user (synced user).
        ...
        # Ensure that event was successfully created in Thrive (no ACL error was triggered blocking creation).
    ...
    def test_create_simple_event_from_outlook_without_organizer(self, mock_get_events):
        """
        Allow creation of an event without organizer in Outlook and sync it in Thrive.
        """
    ...
    def test_create_event_with_default_and_undefined_sensitivity(self):
        """ Check if microsoft events are created in Thrive when 'None' sensitivity setting is defined and also when it is not. """
        # Sync events from Microsoft to Thrive Bureau ERP after adding the sensitivity (privacy) property.
        ...
        self.env['calendar.event']._sync_microsoft2thrive(MicrosoftEvent([undefined_privacy_event, default_privacy_event]))

        # Ensure that synced events have the correct privacy field in Thrive.
        undefined_privacy_thrive_event = self.env['calendar.event'].search([('microsoft_id', '=', 100)])
        default_privacy_thrive_event = self.env['calendar.event'].search([('microsoft_id', '=', 200)])
        self.assertFalse(undefined_privacy_thrive_event.privacy, "Event with undefined privacy must have False value in privacy field.")
        self.assertFalse(default_privacy_thrive_event.privacy, "Event with custom privacy must have False value in privacy field.")

    @patch.object(MicrosoftCalendarService, 'get_events')
    @patch.object(MicrosoftCalendarService, 'insert')
    def test_create_videocall_sync_microsoft_calendar(self, mock_insert, mock_get_events):
        """
        Test syncing an event from Thrive Bureau ERP to Microsoft Calendar.
        ...
        # Verify the event's videocall_location is updated in Thrive
...
class TestSyncThrive2MicrosoftMail(TestCommon, MailCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
        for n in range(1, 3):
            user = cls.env['res.users'].create({
                'name': f'user{n}',
                'login': f'user{n}',
                'email': f'user{n}@thrivebureau.com',
