...
from thrive.addons.hw_drivers.browser import Browser, BrowserState
...
from thrive.addons.hw_drivers.tools.helpers import Orientation
        ...
        if helpers.get_thrive_server_url():
            ...
            try:
                response = http.request(
                    'GET',
                    "%s/iot/box/%s/display_url" % (helpers.get_thrive_server_url(), helpers.get_mac_address())
                )
...
class DisplayController(http.Controller):
    ...
    def customer_facing_display(self, action, pos_id=None, access_token=None, data=None):
        ...
        if action in ['open', 'open_kiosk']:
            origin = helpers.get_thrive_server_url()
