...
from thrive import _, http, release, service
...
def check_git_branch():
    ...
    try:
        ...
        if response.status == 200:
            ...
            _logger.info(
                "Current IoT Box local git branch: %s / Associated Thrive database's git branch: %s",
                local_branch,
                db_branch,
            )
            ...
            if db_branch != local_branch:
                with writable():
                    ...
                    _logger.info("Updating thrive folder to the branch %s", db_branch)
                    subprocess.run(
                        ['/home/pi/thrive/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh'], check=True
                    )
                    thrive_restart()
    except Exception:
...
def save_conf_server(url, token, db_uuid, enterprise_code):
    """
    Save server configurations in thrive.conf
...
def get_thrive_server_url():
    """Get the URL of the linked Thrive database.
    If the IoT Box is in access point mode, it will return ``None`` to avoid
    connecting to the server.

    :return: The URL of the linked Thrive database.
    :rtype: str or None
    """
...
def get_commit_hash():
    return subprocess.run(
        ['git', '--work-tree=/home/pi/thrive/', '--git-dir=/home/pi/thrive/.git', 'rev-parse', '--short', 'HEAD'],
...
def get_version(detailed_version=False):
    if platform.system() == 'Linux':
        image_version = read_file_first_line('/var/thrive/iotbox_version')
    ...
    try:
        for directory in ['drivers', 'interfaces']:
            ...
            for file in iot_handlers:
                unlink_file(f"thrive/addons/hw_drivers/iot_handlers/{directory}/{file}")
...
def compute_iot_handlers_addon_name(handler_kind, handler_file_name):
    return "thrive.addons.hw_drivers.iot_handlers.{handler_kind}.{handler_name}".\
...
def thrive_restart(delay=0):
    """
    Restart Thrive service
...
def update_conf(values, section='iot.box'):
    """
    Update thrive.conf with the given key and value.
    ...
    """
    _logger.debug("Updating thrive.conf with values: %s", values)
    conf = get_conf()
    get_conf.cache_clear()  # Clear the cache to get the updated config

    if not conf.has_section(section):
        _logger.debug("Creating new section '%s' in thrive.conf", section)
        conf.add_section(section)

    for key, value in values.items():
        conf.set(section, key, value) if value else conf.remove_option(section, key)

    write_file("thrive.conf", conf)


@cache
def get_conf(key=None, section='iot.box'):
    """
    Get the value of the given key from thrive.conf, or the full config if no key is provided.
    ...
    conf.read(path_file("thrive.conf"))
...
def disconnect_from_server():
    ...
    get_thrive_server_url.cache_clear()
    update_conf({
        'remote_server': '',
        'token': '',
        'db_uuid': '',
