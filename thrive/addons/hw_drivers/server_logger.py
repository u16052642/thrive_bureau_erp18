...
from thrive.netsvc import DBFormatter
from thrive.tools import config

_logger = logging.getLogger(__name__)

IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in thrive.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, thrive_server_url, active):
        """
        :param thrive_server_url: Thrive Bureau ERP server URL
        """
        super().__init__()
        self._thrive_server_url = thrive_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._thrive_server_url:
    ...
    def _periodic_flush(self):
        thrive_session = requests.Session()
        while self._thrive_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(thrive_session)

    def _flush_logs(self, thrive_session):
        ...
        if not self._thrive_server_url or queue_size == 0:
            return
        try:
            thrive_session.post(
                self._thrive_server_url + '/iot/log',
                data=empty_queue(),
...
def get_thrive_config_log_to_server_option():
    return config.get(IOT_LOG_TO_SERVER_CONFIG_NAME, True)  # Enabled by default


def check_and_update_thrive_config_log_to_server_option(new_state):
    """
    :return: wherever the config file need to be updated or not
    """
    if get_thrive_config_log_to_server_option() != new_state:
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_thrive_server_url(), get_thrive_config_log_to_server_option())
...
# Set it in the 'root' logger, on which every logger (including thrive) is a child
