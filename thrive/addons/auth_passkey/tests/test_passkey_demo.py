...
from thrive.tools import SQL, mute_logger
...
class PasskeyTest(HttpCase):
    ...
    def test_check_identity(self):
        for key in ['test-yubikey', 'test-yubikey-nano', 'test-keepassxc']:
            ...
            with self.patch_start_auth(webauthn_challenge):
                ...
                self.assertEqual(response['error']['data']['name'], 'thrive.exceptions.UserError')
                self.assertEqual(
    ...
    def test_check_user_verification(self):
        ...
        with self.patch_start_auth(webauthn_challenge):
            ...
            # Login successful, redirected to /thrive
            self.assertTrue(response.url.endswith('/thrive'))
