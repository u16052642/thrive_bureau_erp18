...
class FetchmailServer(models.Model):
            ...
            if float_compare(move.amount_total, xml_total_amount, precision_digits=move.currency_id.decimal_places) != 0:
                move.message_post(
                    body=Markup(_('<strong>Warning:</strong> The total amount of the DTE\'s XML is %(xml_total_amount)s and the total amount '
                           'calculated by Thrive is %(move_total_amount)s. Typically this is caused by additional lines in the detail or '
    ...
    def _get_dte_lines(self, dte_xml, company_id, partner_id):
        ...
        for dte_line in dte_xml.findall('.//ns0:Detalle', namespaces=XML_NAMESPACES):
            ...
            # by the quantity we get the price unit we should use in Thrive.
            values = {
                'product': product,
