...
import { stores, helpers } from "@thrive/o-spreadsheet";
...
describe("field sync action", () => {
    ...
    test("auto resize list columns", async () => {
        ...
        expect(getCellContent(model, "A1")).toBe('=THRIVE.LIST.HEADER(1,"product_uom_qty")');
    ...
    test("hover field sync highlights matching list formulas", async () => {
        ...
        setCellContent(model, "A1", '=THRIVE.LIST(1,1,"product_uom_qty")');
        ...
        setCellContent(model, "A1", "=THRIVE.LIST(A2, A3, A4)");
        expect(highlightStore.highlights).toHaveLength(0);
