...
class AccountMove(models.Model):
    ...
    def action_l10n_vn_edi_update_payment_status(self):
        ...
        for invoice in self.filtered(lambda i: i.l10n_vn_edi_invoice_state == 'payment_state_to_update'):
            ...
            # SInvoice will return a NOT_FOUND_DATA error if the status in Thrive matches the one on their side.
        ...
        try:
            ...
            self.with_context(no_new_invoice=True).message_post(
                body=_('The invoice has been canceled on sinvoice for reason: %(reason)s'
                       'But the cancellation in Thrive failed with error: %(error)s', reason=reason, error=e),
            )
