...
class ResCompany(models.Model):
    ...
    l10n_ke_oscu_user_agreement = fields.Boolean(
        string="Thrive OSCU user agreement",
        help="Agreement is required to use Thrive as an OSCU service provider.",
        tracking=True,
    )
    ...
    def _compute_l10n_ke_oscu_serial_number(self):
        for company in self.filtered(lambda c: c.country_code == 'KE'):
            company.l10n_ke_oscu_serial_number = f'THRIVE/{company.vat}/0'
    ...
    def write(self, vals):
        ...
        if email_is_not_sent:
            for company in email_is_not_sent.filtered(lambda c: c.l10n_ke_oscu_user_agreement and c.l10n_ke_oscu_cmc_key):
                logging_message = f"""
                    Checkbox `Thrive OSCU user agreement` was set to True
                ...
                mail_body = Markup("""
                    <div style="margin: 0px; padding: 0px;">
                        <p style="margin: 0px; padding: 0px;">
                            Dear %s,
                            <br/>
                            This is a notification that you have agreed to Thrive Bureau ERP's OSCU user agreement. %s can now
                            use OSCU flows in Thrive to declare your activities with the KRA.
                            <br/>
                            This is an automated e-mail and no further action is needed on your part.
                            <br/>
                            Thank you for choosing Thrive for your Kenyan ERP needs.
                        </p>
                    </div>
                """) % (self.env.user.name, company.name)
                mail_subject = f" {company.name} - Thrive OSCU User Agreement confirmation"
                mail_values = {
    ...
    def _l10n_ke_preinitialization_checks(self):
        ...
        if not self.l10n_ke_oscu_user_agreement:
            raise UserError(_("Please agree to the terms of use of Thrive as an OSCU service provider first. "))
        error_fields = []
        on_company = False
        if not self.vat:
        ...
        if not self.l10n_ke_oscu_serial_number.upper().startswith('THRIVE/' + self.vat.upper() + '/'):
            raise UserError(_('Your serial number should contain the PIN number and start: THRIVE/%s/', self.vat))
    ...
    def action_l10n_ke_create_branches(self):
        """ Query eTIMS for saved company branches, then create those branches in Thrive. """
