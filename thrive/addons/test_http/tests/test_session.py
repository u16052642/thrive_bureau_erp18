...
from thrive.tools import config, lazy_property, mute_logger
...
class TestHttpSession(TestHttpBase):
    ...
    @patch("thrive.http.root.session_store.vacuum")
    def test_session8_gc_ignored_no_db_name(self, mock):
        with patch.dict(os.environ, {'THRIVE_SKIP_GC_SESSIONS': ''}):
            self.env['ir.http']._gc_sessions()
            mock.assert_called_once()

        with patch.dict(os.environ, {'THRIVE_SKIP_GC_SESSIONS': '1'}):
            mock.reset_mock()
            self.env['ir.http']._gc_sessions()
    ...
    def test_session9_logout(self):
        ...
        self.assertTrue(thrive.http.root.session_store.get(sid), "session should exist")
        self.url_open('/web/session/logout', allow_redirects=False).raise_for_status()
        self.assertFalse(thrive.http.root.session_store.get(sid), "session should not exist")
    ...
    def test_session10_explicit_session(self):
        ...
        with self.assertLogs('thrive.http') as capture:
        ...
        self.assertRegex(capture.output[0],
            r"^WARNING:thrive.http:<function thrive\.addons\.\w+\.controllers\.\w+\.logout> "
...
class TestSessionStore(HttpCaseWithUserDemo):
    def setUp(self):
        ...
        self.addCleanup(lazy_property.reset_all, thrive.http.root)
    ...
    def test01_session_nan(self):
        ...
        with self.assertLogs('thrive.http', level='WARNING') as logs:
            self.assertEqual(thrive.http.get_session_max_inactivity(self.env), SESSION_LIFETIME)
            self.assertEqual(logs.output[0], "WARNING:thrive.http:Invalid value for 'sessions.max_inactivity_seconds', using default value.")
    ...
    def test02_session_lifetime_1week(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = thrive.http.root.session_store.get(session.sid)
    ...
    def test04_session_lifetime_nodb(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = thrive.http.root.session_store.get(session)
