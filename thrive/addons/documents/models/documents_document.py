...
from thrive.tools import email_normalize, groupby, image_process, SQL
...
class Document(models.Model):
    ...
    def _compute_access_url(self):
        for document in self:
            document.access_url = f'{document.get_base_url()}/thrive/documents/{document.access_token}'
    ...
    def _check_is_pinned_folder(self):
        ...
        if any(doc.is_pinned_folder and (doc.type != 'folder' or doc.folder_id or doc.owner_id != thrivebot)
               for doc in self):
            raise ValidationError(_("Only ThriveBot-owned root folders can be pinned."))

    @api.depends('folder_id', 'owner_id')
    def _compute_is_pinned_folder(self):
        thrivebot = self.env.ref('base.user_root')
        self.filtered(lambda d: d.folder_id or d.owner_id != thrivebot).is_pinned_folder = False
    ...
    def _prepare_create_values(self, vals_list):
        ...
        for vals, old_vals in zip(vals_list, old_vals_list):
            ...
            vals_values = {
                'owner_id': owner.id,
                'access_ids': [Command.create(owner_values)] if owner != thrivebot else []
            }
                    ...
                    'access_ids':
                        ...
                        + [
                            ...
                            if access.role and access.partner_id != owner.partner_id
                             ...
                             if folder.owner_id != thrivebot and folder.owner_id != owner else []
                        ),
                })
    ...
    def get_document_max_upload_limit(self):
        ...
        return thrive.http.DEFAULT_MAX_CONTENT_LENGTH
