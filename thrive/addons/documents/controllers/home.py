...
class Home(web_home.Home):
    def _web_client_readonly(self):
        ...
        if (
            path.startswith('/thrive/documents')
            and (request.httprequest.args.get('access_token') or path.removeprefix('/thrive/documents/'))
    ...
    def web_client(self, s_action=None, **kw):
        """ Handle direct access to a document with a backend URL (/thrive/documents/<access_token>).
        ...
        if not document_sudo:
            ...
            if document_sudo := Redirect._get_redirection(access_token):
                return request.redirect(
                    f'/thrive/documents/{document_sudo.access_token}',
        ...
        # /thrive/documents/<id> to /thrive/documents/<access-token> in the
        # URL.
