...
from thrive.addons.documents.tests.test_documents_common import TransactionCaseDocuments
...
class TestDocumentsAccess(TransactionCaseDocuments):
    ...
    def test_restrict_write_on_pinned_folders(self):
        ...
        """
        thrivebot = self.env.ref('base.user_root')
        self.assertFalse(self.folder_a.folder_id)
        self.folder_a.owner_id = thrivebot
        ...
        # Managers can unpin by moving to another thrivebot folder
        self.folder_b.owner_id = thrivebot
    ...
    def test_pin_folder_folder_id(self):
        ...
        # manager can do what they want with an thrivebot folder
    ...
    def test_unlink_with_children(self):
        ...
        important to make sure that an ThriveBot-owned 2nd level folder cannot be
    ...
    def test_access_rights_shortcuts_propagation(self):
        ...
        root = self.env['documents.document'].create({
            ...
            'owner_id': self.document_manager.id  # not thrivebot
        })
    ...
    def test_copy_document_access(self):
        ...
        url_document_in_my_folder = self.env['documents.document'].create(
            {'name': 'url', 'type': 'url', 'url': 'https://www.thrivebureau.com/', 'owner_id': self.internal_user.id})
