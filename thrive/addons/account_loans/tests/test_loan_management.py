...
from thrive.addons.account_loans import _account_loans_add_date_column
...
class TestLoanManagement(AccountTestInvoicingCommon):
    ...
    def test_loan_values(self):
        ...
        # Create the loan
        loan = self.create_loan('Thrivemobile Loan 🚗', '2024-01-01', 2 * 12, 24_000, 2_400, validate=True)
        ...
        self.assertRecordValues(payment_moves[0] | payment_moves[11] | payment_moves[15] | payment_moves[-1], [{
            ...
            'ref': "Thrivemobile Loan 🚗 - Principal & Interest 01/2024",
        ...
        self.assertRecordValues(reclassification_moves[0] | reclassification_moves[11] | reclassification_moves[15], [{
            'date': fields.Date.to_date('2024-01-31'),
            'ref': "Thrivemobile Loan 🚗 - Reclassification LT - ST 02/2024 to 01/2025",  # offset of 1 month
            'amount_total': 12_000,  # sum of the principals of the next 12 months
            'generating_loan_line_id': loan.line_ids[0].id,
        }, {
            'date': fields.Date.to_date('2024-12-31'),
            'ref': "Thrivemobile Loan 🚗 - Reclassification LT - ST 01/2025 to 12/2025",  # offset of 1 month
            'amount_total': 12_000,  # sum of the principals between 01/25 and 12/25
            'generating_loan_line_id': loan.line_ids[11].id,
        }, {
            'date': fields.Date.to_date('2025-04-30'),
            'ref': "Thrivemobile Loan 🚗 - Reclassification LT - ST 05/2025 to 12/2025",  # offset of 1 month
        ...
        self.assertRecordValues(reclassification_moves[0].line_ids.sorted(lambda l: l.credit), [{
            'name': f'Thrivemobile Loan 🚗 - Reclassification LT - ST 02/2024 to 01/2025 (To {self.short_term_account.code})',
            'debit': 12_000,
            'credit': 0,
            'account_id': self.long_term_account.id,
        }, {
            'name': f'Thrivemobile Loan 🚗 - Reclassification LT - ST 02/2024 to 01/2025 (From {self.long_term_account.code})',
            'debit': 0,
            'credit': 12_000,
            'account_id': self.short_term_account.id,
        }])
    ...
    def test_loan_states(self):
        ...
        # Create the loan
        loan = self.create_loan('Thrivemobile Loan 🚗', '2024-01-01', 12, 24_000, 2_400)
        ...
        loan2 = self.create_loan('Thrivemobile Loan 🚗', '2024-01-01', 12, 24_000, 2_400, validate=True)
