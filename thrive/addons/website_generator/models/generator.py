...
from thrive.tools import LazyTranslate
...
class WebsiteGeneratorRequest(models.Model):
    ...
    def _generate_site(self, tar):
        ...
        thrive_blocks['direct_html_replacements_mapping'] = {}
        thrive_blocks['regex_html_replacements_mapping'] = {}
        ...
        self._create_model_records(tar, thrive_blocks)
        # Create redirects, modifies thrive_blocks in place
        self._apply_all_html_replacements(thrive_blocks)
        self._generate_pages(website, thrive_blocks)
        return website, thrive_blocks

    def _apply_all_html_replacements(self, thrive_blocks):
        direct_html_replacements_mapping = thrive_blocks.get('direct_html_replacements_mapping', {})
        regex_html_replacements_mapping = thrive_blocks.get('regex_html_replacements_mapping', {})
        ...
        for page_name, page_dict in thrive_blocks.get('pages', {}).items():
            thrive_blocks['pages'][page_name]['body_html'] = self._apply_html_replacements(page_dict.get('body_html', []), sorted_original_html, direct_html_replacements_mapping, regex_html_replacements_mapping)

    def _create_model_records(self, tar, thrive_blocks):
    ...
    def _save_images_as_attachments(self, thrive_blocks, tar):
        ...
        thrive_blocks['direct_html_replacements_mapping'].update({html.escape(k): v.image_src for k, v in attachments_url_src.items()})

        # Create attachments for all images (cropped)
        customized_attachments_url_src = {}
        customized_image_mappings = {}
        for page_dict in [thrive_blocks['homepage']] + list(thrive_blocks.get('pages', {}).values()):
        ...
        thrive_blocks['regex_html_replacements_mapping'].update(customized_image_mappings)
    ...
    def convert_scraping_request_ICP(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': "/thrive/action-website_generator.website_generator_screen?reload=true",
            'target': 'self',
        }
