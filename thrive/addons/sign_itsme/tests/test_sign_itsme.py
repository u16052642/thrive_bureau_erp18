...
from thrive.addons.sign.tests.test_sign_controllers import TestSignControllerCommon
...
class SignItsmeCommon(TestSignControllerCommon):
    ...
    def test_sign_itsme_API(self):
        ...
        with patch('thrive.addons.iap.models.iap_account.IapAccount.get_credits', lambda self, x: 10):
            with patch('thrive.addons.sign_itsme.controllers.main.jsonrpc', lambda url, params: {'success': True, 'url': url}):
    ...
    def test_sign_itsme_request_allows_no_auth_when_credits_are_not_available(self):
        ...
        sign_vals = self.create_sign_values(sign_request.template_id.sign_item_ids, sign_request_item.role_id.id)
        with patch('thrive.addons.iap.models.iap_account.IapAccount.get_credits', lambda self, service: 0):
            response = self._json_url_open(
                '/sign/sign/%d/%s' % (sign_request.id, sign_request_item.access_token),
                data={'signature': sign_vals},
                headers={"Referer": 'abc'}
