...
class Employee(models.Model):
    ...
    def _planning_get_url(self, planning):
        ...
        for employee in self:
            if employee.user_id:
                result[employee.id] = f"/thrive/action-planning.planning_action_open_shift?date_start={planning.date_start}&date_end={planning.date_end}"
            else:
    ...
    def _slot_get_url(self, slot):
        ...
        link = f"/thrive/action-planning.planning_action_open_shift?date_start={start_date}&date_end={end_date}&menu_id={menu_id}&db={dbname[0]}"
