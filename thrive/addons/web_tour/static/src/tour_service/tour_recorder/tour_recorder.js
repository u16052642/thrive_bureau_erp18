...
import { queryAll, queryFirst, queryOne } from "@thrive/hoot-dom";
...
const THRIVE_CLASS_REGEX = /^oe?(-|_)[\w-]+$/;
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasThriveClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasThriveClass;
        ...
        const thriveClass = [...currentElem.classList].find((c) => c.match(THRIVE_CLASS_REGEX));
        if (thriveClass) {
            currentPredicate = `.${thriveClass}`;
            hasThriveClass = true;
        }
