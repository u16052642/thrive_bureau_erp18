...
import { afterEach, beforeEach, describe, expect, test } from "@thrive/hoot";
import { Component, useState, xml } from "@thrive/owl";
import { advanceTime, animationFrame, runAllTimers } from "@thrive/hoot-mock";
import { click, queryFirst, waitFor } from "@thrive/hoot-dom";
...
test("a failing tour logs the step that failed in run", async () => {
    ...
    await thrive.startTour("tour2", { mode: "auto" }); // Use thrive to run tour from registry because this is a test tour
...
test("a failing tour with disabled element", async () => {
    ...
    await thrive.startTour("tour3", { mode: "auto" });
    await advanceTime(750);
    await advanceTime(750);
    await advanceTime(750);
    await advanceTime(750);
...
test("a failing tour logs the step that failed", async () => {
    ...
    await thrive.startTour("tour1", { mode: "auto" });
    await advanceTime(750);
...
test("check tour with inactive steps", async () => {
    ...
    await thrive.startTour("pipu_tour", { mode: "auto" });
    await advanceTime(750);
    await advanceTime(750);
    await advanceTime(750);
...
test("automatic tour with invisible element", async () => {
    ...
    await thrive.startTour("tour_de_wallonie", { mode: "auto" });
    await animationFrame();
    await advanceTime(750);
    await advanceTime(750);
    await advanceTime(750);
...
test("automatic tour with alternative trigger", async () => {
    ...
    await mountWithCleanup(Root);
    await thrive.startTour("tour_des_flandres", { mode: "auto" });
    for (let i = 0; i <= 5; i++) {
        await advanceTime(750);
    }
    await advanceTime(10000);
