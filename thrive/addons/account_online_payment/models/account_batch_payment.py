...
from thrive.addons.account.tools.structured_reference import is_valid_structured_reference
...
class AccountBatchPayment(models.Model):
    ...
    def initiate_payment(self):
        ...
        - Signing the payment online through Thrivefin
    ...
    def validate_batch(self):
        ...
        if self.payment_method_code == 'sepa_ct' and self.account_online_linked:
            ...
            while True:
                response = account_online_link._fetch_thrive_fin('/proxy/v1/initiate_payment', data)
    ...
    def check_online_payment_status(self):
        ...
        for batch in self:
            ...
            while True:
                response = batch.journal_id.account_online_link_id._fetch_thrive_fin('/proxy/v1/get_payment_status', data)
                # In case of token expiration, we receive a special next_data field that we use to redo the request
                if not response.get('next_data'):
                    break
                data['next_data'] = response['next_data']
    ...
    def _sign_payment(self):
        ...
        while True:
            response = account_online_link._fetch_thrive_fin('/proxy/v1/sign_payment', data)
