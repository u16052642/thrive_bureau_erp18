...
from thrive.http import Response, request
...
from thrive.addons.hw_drivers.server_logger import (
    check_and_update_thrive_config_log_to_server_option,
    close_server_log_sender_handler,
    get_thrive_config_log_to_server_option,
)
...
class IoTboxHomepage(Home):
    ...
    def index(self):
        ...
        remote_server = helpers.get_thrive_server_url()
    ...
    def list_handlers(self, **post):
        ...
        if request.httprequest.method == 'POST':
            ...
            need_config_save |= check_and_update_thrive_config_log_to_server_option(
            ...
            for post_request_key, log_level_or_parent in post.items():
                ...
                elif post_request_key == 'thrive':
                    need_config_save |= self._update_logger_level('thrive', log_level_or_parent, AVAILABLE_LOG_LEVELS)
        ...
        return handler_list_template.render({
            ...
            'is_log_to_server_activated': get_thrive_config_log_to_server_option(),
            'root_logger_log_level': self._get_logger_effective_level_str(logging.getLogger()),
            'thrive_current_log_level': self._get_logger_effective_level_str(logging.getLogger('thrive')),
    ...
    def connect_to_server(self, token, iotname):
        ...
        helpers.thrive_restart(1)  # 1 sec delay for IO operations (save_conf_server)
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get Thrive Iot logger given an IoT handler name
        ...
        """
        thrive_addon_handler_path = helpers.compute_iot_handlers_addon_name(handler_folder_name, handler_name)
        return thrive_addon_handler_path in logging.Logger.manager.loggerDict.keys() and \
               logging.getLogger(thrive_addon_handler_path)

    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """
        Update (if necessary) Thrive's configuration and logger to the given logger_name to the given level.
        ...
        THRIVE_TOOL_CONFIG_HANDLER_NAME = 'log_handler'
        LOG_HANDLERS = tools.config[THRIVE_TOOL_CONFIG_HANDLER_NAME]
        ...
        tools.config[THRIVE_TOOL_CONFIG_HANDLER_NAME] = log_handlers_without_logger
        new_level_upper_case = new_level.upper()
        if not IS_NEW_LEVEL_PARENT:
            new_entry = [LOGGER_PREFIX + new_level_upper_case]
            tools.config[THRIVE_TOOL_CONFIG_HANDLER_NAME] += new_entry
            _logger.debug('Adding to thrive config log_handler: %s', new_entry)
