...
import { isIconElement, isSelectionInSelectors, peek } from '@web_editor/js/editor/thrive-editor/src/utils/utils';
...
const childNodeIndex = ThriveEditorLib.childNodeIndex;
const fillEmpty = ThriveEditorLib.fillEmpty;
const isVisible = ThriveEditorLib.isVisible;
const getDeepestPosition = ThriveEditorLib.getDeepestPosition;
const paragraphRelatedElements = ThriveEditorLib.paragraphRelatedElements;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                try {
                    ...
                    this.thriveEditor._computeHistorySelection();
                } finally {
        ...
        const colorPaletteCommonOptions = {
            ...
            onColorLeave: () => {
                ...
                this.thriveEditor.withoutRollback(() => this.thriveEditor.historyRevertCurrentStep());
    ...
    async startEdition() {
        ...
        if ($('wrapwrap').length) {
            this.multiSelectionTarget = $().getScrollingTarget(this.thriveEditor.document);
            this.multiSelectionTarget.addEventListener('scroll', this.thriveEditor.multiselectionRefresh, { passive: true });
        }
    ...
    destroy() {
        ...
        this.multiSelectionTarget?.removeEventListener('scroll', this.thriveEditor.multiselectionRefresh, { passive: true });
    ...
    getValue(options) {
        ...
        if (this.thriveEditor) {
            this.thriveEditor.cleanForSave($editable[0]);
    ...
    _observeThriveFieldChanges() {
        ...
        if (this.thriveFieldObservers) {
            ...
            const thriveFieldSelector = '[data-oe-model], [data-oe-translation-source-sha]';
            const $thriveFields = this.$editable.find(thriveFieldSelector);
            const renderingClassesSelector = this.thriveEditor.options.renderingClasses
            ...
            $thriveFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    ...
                    for (const node of fieldNodeClone.querySelectorAll(renderingClassesSelector)) {
                        node.classList.remove(...this.thriveEditor.options.renderingClasses);
                    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        if (this.state.showSnippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                this.thriveEditor.document.removeEventListener('click', this._onClick, true);
                ...
                this.thriveEditor.document.addEventListener('click', this._onClick, true);
    ...
    _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.thriveEditor.editable.focus();
        } else {
    ...
    _configureToolbar(options) {
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            const destroyTransfo = () => {
                ...
                this.thriveEditor.document.removeEventListener('keydown', keydown);
            }
            const mouseup = () => {
            ...
            this.thriveEditor.document.addEventListener('keydown', keydown);
        });
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const selectedTds = this.thriveEditor.document.querySelectorAll('td.o_selected_td');
        ...
        let coloredElements = this.thriveEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.thriveEditor.document.contains(element));
        ...
        if (coloredTds.length) {
            ...
            range.setEnd(...endPos(last));
                const { startContainer, startOffset, endContainer, endOffset } = getDeepRange(this.thriveEditor.editable, { range });
            if (isSelForward) {
    ...
    _getBannerCommand(title, emoji, alertClass, iconClass, description, priority) {
        return {
            ...
            callback: () => {
                ...
                this.thriveEditor.activateContenteditable();
            ...
            },
            {
                ...
                callback: async () => {
                    ...
                    if (currentUser && currentUser.signature) {
                        this.thriveEditor.execCommand('insert', parseHTML(this.thriveEditor.document, currentUser.signature));
                    }
                },
            },
            {
    ...
    _onSelectionChange() {
        if (this.thriveEditor.autohideToolbar && this.linkPopover) {
            const selectionInLink = getInSelection(this.thriveEditor.document, 'a') === this.linkPopover.target;
    ...
    _beforeAnyCommand() {
        ...
        for (const node of ThriveEditorLib.getSelectedNodes(this.$editable[0])) {
