...
import { Tooltip as ThriveTooltip } from "@web/core/tooltip/tooltip";
...
class SnippetsMenu extends Component {
    ...
    setup() {
        ...
        // Thrive Editor uses the HTML Element to bind commands.
        ...
        onMounted(async () => {
            ...
            this.options.wysiwyg.thriveEditor.bindExecCommand(titleButtons);

            // Get table container and bind commands to Thrive Bureau ERP Editor.
    ...
    async start() {
        ...
        return Promise.all(defs).then(() => {
            const updateHistoryButtons = () => {
                this.state.canRedo = this.options.wysiwyg.thriveEditor.historyCanRedo();
                this.state.canUndo = this.options.wysiwyg.thriveEditor.historyCanUndo();
            };
    ...
    _onClick(ev) {
        ...
        if ($oeStructure.length && !$oeStructure.children().length && this.snippets.size > 0) {
            ...
            this._activateSnippet(false).then(() => {
                this.$el.find('.oe_snippet').thriveBounce();
            });
            return;
        }
        this._activateSnippet($target);
   ...
   _onSnippetDragAndDropStart() {
    ...
    _showSnippetTooltip(snippetEl, delay = 1500) {
        ...
        this.hideShownTooltip = this.popover.add(snippetEl, ThriveTooltip, {
    ...
    _onSaveRequest(ev) {
        ...
        // If it's an ThriveEvent sent by sub-widgets, we prevent the event
    ...
    async _openAddSnippetDialog(snippetGroup = null, initialSnippetEl) {
        this._mutex.exec(async () => {
            ...
            this.options.wysiwyg.thriveEditor.historyPauseSteps();
            ...
            if (!hasIncludedSnippet) {
                ...
                this.options.wysiwyg.thriveEditor.historyUnpauseSteps();
    ...
    _installModule(moduleID, snippetName) {
        ...
        const linkUrl = '/thrive/action-base.open_module_tree/' + encodeURIComponent(moduleID);
