...
export function loadImage(src, img = new Image()) {
    ...
    // grep: LOAD_IMAGE_404
    const placeholderHref = "/web/image/__thrive__unknown__src__/";
...
export async function loadImageInfo(img, attachmentSrc = '') {
    ...
    // looks like "https://mycompany.thrivebureau.com/web/image/123" that leads to a
...
export async function isImageCorsProtected(img) {
    ...
    if (!src.startsWith("/") || /\/web\/image\/\d+-redirect\//.test(src)) {
        ...
        //    For instance, abc.thrivebureau.com vs abc.com which are actually the
