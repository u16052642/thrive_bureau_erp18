    ...
    async addElement(previewMode, widgetValue, params) {
        ...
        if (elementType === 'image') {
            ...
            if (!isImageSaved) {
                ...
                // media dialog closed.
                this.options.wysiwyg.thriveEditor.historyRevertCurrentStep();
                return;
            }
...
￼    */
    ...
    async _writeShape(svgText) {
        ...
        if (hasHoverEffect) {
            ...
            clonedImgEl = img.cloneNode(true);
            this.options.wysiwyg.thriveEditor.observerUnactive("addClonedImgForHoverEffectPreview");
            img.classList.add("d-none");
            img.insertAdjacentElement("afterend", clonedImgEl);
            this.options.wysiwyg.thriveEditor.observerActive("addClonedImgForHoverEffectPreview");
        }
        const loadedImg = await loadImage(dataURL, img);
        if (hasHoverEffect) {
            this.options.wysiwyg.thriveEditor.observerUnactive("removeClonedImgForHoverEffectPreview");
            clonedImgEl.remove();
            img.classList.remove("d-none");
            this.options.wysiwyg.thriveEditor.observerActive("removeClonedImgForHoverEffectPreview");
        }
...
registry.BackgroundImage = SnippetOptionWidget.extend({
    ...
    _setBackground(backgroundURL) {
        ...
        this.options.wysiwyg.thriveEditor.editable.focus();
    },
});
