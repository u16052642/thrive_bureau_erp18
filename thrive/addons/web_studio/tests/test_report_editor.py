...
from thrive.addons.web_studio.controllers.report import WebStudioReportController, get_report_view_copy, _get_and_write_studio_view
...
class TestReportEditorUIUnit(HttpCase):
    ...
    @property
    def tour_url(self):
        return f"/thrive/action-studio?mode=editor&_action={self.testAction.id}&_tab=reports&_report_id={self.report.id}&menu_id={self.testMenu.id}"
    ...
    def test_record_model_differs_from_action(self):
        ...
        self.start_tour(f"/thrive/action-studio?mode=editor&_action={self.testAction.id}&_tab=reports&menu_id={self.testMenu.id}", "web_studio.test_record_model_differs_from_action", login="admin")
