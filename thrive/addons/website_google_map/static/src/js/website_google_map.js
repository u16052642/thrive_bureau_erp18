...
function initialize_map() {
    ...
    async function initializeMarkers(thrivePartnerData) {
        // Create the markers and cluster them on the map
        if (thrivePartnerData) { /* thrive_partner_data special variable should have been defined in google_map.xml */
            const markerPromises = [];
            for (var i = 0; i < thrive_partner_data.counter; i++) {
                markerPromises.push(set_marker(thrive_partner_data.partners[i]));
            }
    ...
    initializeMarkers(thrive_partner_data);
