...
from thrive.tools.mimetypes import fix_filename_extension, get_extension, guess_mimetype
...
class test_guess_mimetype(BaseCase):
    ...
    def test_mimetype_fix_extension(self):
        ...
        with self.assertLogs('thrive.tools.mimetypes', 'WARNING') as capture:
            self.assertEqual(fix('image.txt', 'image/jpeg'), 'image.txt.jpg')
            self.assertEqual(fix('words.jpg', 'text/plain'), 'words.jpg.txt')
        self.assertEqual(capture.output, [
            "WARNING:thrive.tools.mimetypes:File 'image.txt' has an invalid "
                "extension for mimetype 'image/jpeg', adding '.jpg'",
            "WARNING:thrive.tools.mimetypes:File 'words.jpg' has an invalid "
