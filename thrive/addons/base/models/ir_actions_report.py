...
from thrive.exceptions import UserError, AccessError, RedirectWarning
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start Thrive with at least two workers to convert images to html.')
    else:
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        if error_record_ids:
            ...
            raise RedirectWarning(
                message=_('Thrive is unable to merge the generated PDFs because of %(num_errors)s corrupted file(s)', num_errors=num_errors),
                action=action,
