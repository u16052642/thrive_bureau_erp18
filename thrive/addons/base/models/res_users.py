...
from thrive.tools import is_html_empty, partition, frozendict, lazy_property, SQL, SetDefinitions
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "https://www.thrivebureau.com/documentation/master/administration/install/deploy.html#https for details.",
                    source
                )
...
class GroupsImplied(models.Model):
    ...
    def _get_group_definitions(self):
        """ Return the definition of all the groups as a :class:`~thrive.tools.SetDefinitions`. """
