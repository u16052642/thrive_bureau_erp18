...
const articleCommandSteps = [
    { // open the command bar
        trigger: `[name="body"] .thrive-editor-editable > p:last-child`,
        run: function () {
...
const embedKanbanEditArticleSteps = [{ // Create a new article using quick create in OnGoing Column
    ...
    trigger: '.thrive-editor-editable:contains("EditorCommandsArticle Content")',
}];
...
const articleCommandComposerSteps = [{ // open the chatter
    ...
    trigger: `${composeBody} .thrive-editor-editable > p > a:nth-child(2).o_knowledge_article_link:contains("LinkedArticle")[contenteditable="false"]`,
}, { // verify that the first block is still there and contenteditable=false
    trigger: `${composeBody} .thrive-editor-editable > p > a:nth-child(1).o_knowledge_article_link:contains("EditorCommandsArticle")[contenteditable="false"]`,
}, { // send the message
    trigger: '.o_mail_send',
    run: "click",
}, {
