...
# Thrive
thrive_url = 'http://localhost:8069'
thrive_db = 'thrive_db'
thrive_username = 'admin'
thrive_password = 'admin'
...
def get_blobs_to_be_deleted(blob_urls, batch_size=1000):
    common = xmlrpc.client.ServerProxy(f'{thrive_url}/xmlrpc/2/common')
    uid = common.authenticate(thrive_db, thrive_username, thrive_password, {})
    models = xmlrpc.client.ServerProxy(f'{thrive_url}/xmlrpc/2/object')
    for blob_urls_ in split_every(batch_size, blob_urls):
        blob_urls_ = list(blob_urls_)
        attachments = models.execute_kw(thrive_db, uid, thrive_password, 'ir.attachment', 'search_read', [
