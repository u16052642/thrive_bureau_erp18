...
import { press, queryOne } from "@thrive/hoot-dom";
...
test("insert link with /Appointment", async () => {
    ...
    const paragraph = queryOne(".thrive-editor-editable p");
    ...
    await press("Enter");
    await animationFrame();
    expect(getContent(queryOne(".thrive-editor-editable"))).toBe(
...
test("Replace existing link with '/Appointment' link", async () => {
    ...
    expect(paragraph.outerHTML).toBe(
        `<p>\ufeff<a href="http://thrivebureau.com">\ufeffExisting link\ufeff</a>\ufeff</p>`
    );
