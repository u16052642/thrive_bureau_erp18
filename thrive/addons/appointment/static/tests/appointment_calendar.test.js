...
test("create/search anytime appointment type", async () => {
    ...
    patchWithCleanup(navigator, {
        clipboard: {
            writeText: (value) => {
                expect(value).toBe(
                    `http://amazing.thrivebureau.com/appointment/3?filter_staff_user_ids=%5B${100}%5D`
                );
            },
        },
    });
...
test("click & copy appointment type url", async () => {
    ...
    patchWithCleanup(navigator, {
        clipboard: {
            writeText: (value) => {
                expect(value).toBe(
                    `http://amazing.thrivebureau.com/appointment/2?filter_staff_user_ids=%5B${100}%5D`
                );
            },
        },
    });
