...
class TestIncomingTransactionResponse(BaseCase):
    @patch.dict(
        "sys.modules", {
            ...
            "thrive.addons.hw_drivers": MagicMock(),
            # Mock the modules IngenicoDriver imports so the imports don't fail
            "thrive.addons.hw_drivers.driver": MagicMock(),
            "thrive.addons.hw_drivers.event_manager": MagicMock(),
            "thrive.addons.hw_drivers.iot_handlers.interfaces.SocketInterface": MagicMock(),
        }
    )
    def setUp(self):
        # pylint: disable=import-outside-toplevel
        from thrive.addons.iot.iot_handlers.drivers.IngenicoDriver import IncomingIngenicoMessage
...
class TestOutgoingIngenicoMessage(BaseCase):
    ...
    def setUp(self):
        # pylint: disable=import-outside-toplevel
        from thrive.addons.iot.iot_handlers.drivers.IngenicoDriver import OutgoingIngenicoMessage
