...
from thrive.tools import file_open, mute_logger
...
class TestCaseDocumentsBridgeSign(SignRequestCommon):
    ...
    @mute_logger('thrive.addons.documents.models.documents_document')
    ...
    @mute_logger("thrive.addons.documents.models.documents_document")  # avoid warning about counting page of PDFs
        ...
        for user in users:
            with self.subTest(user_name=user.name):
                ...
                self.assertEqual(documents_signed.owner_id, self.env.ref('base.user_root'),
                                 "Owner of the signed/certificate documents must be thriveBot.")
