...
class ThriveChartFieldMatchingClipboardHandler extends AbstractFigureClipboardHandler {
    copy({ figureId }) {
        if (!this.getters.getChart(figureId)?.type.startsWith("thrive")) {
            return;
        }
        return {
            thriveChartFieldMatching: this.getters.getChartFieldMatch(figureId),
        };
    }

    paste(target, clippedContent, options) {
        const { figureId: newFigureId } = target;
        const clippedMatchings = clippedContent.thriveChartFieldMatching;
        if (!clippedMatchings) {
            return;
        }

        const thriveChartIds = globalFiltersFieldMatchers["chart"].getIds();
        for (const filterId in clippedMatchings) {
            ...
            for (const chartId of thriveChartIds) {
                currentChartMatchings[chartId] = this.getters.getThriveChartFieldMatching(
                    chartId,
                    filterId
                );
            }
...
clipboardHandlersRegistries.figureHandlers.add(
    "thrive_chart_field_matching",
    ThriveChartFieldMatchingClipboardHandler
);
