import { helpers, stores } from "@thrive/o-spreadsheet";
...
export class CommentsStore extends SpreadsheetStore {
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "SET_VIEWPORT_OFFSET":
                if (this.cellPopoverStore.persistentCellPopover?.type === "ThriveCellComment") {
        ...
        if (this.getters.isGridSelectionActive() && !this.getters.isReadonly()) {
            ...
            if (threads && threads.length) {
                ...
                if (!thread.isResolved) {
                    this.cellPopoverStore.open(position, "ThriveCellComment");
    ...
    toggleComments() {
        ...
        if (
            ...
            this.cellPopoverStore.persistentCellPopover?.type === "ThriveCellComment"
        ) {
            this.cellPopoverStore.close();
            this.selectedThreadId = undefined;
        }
