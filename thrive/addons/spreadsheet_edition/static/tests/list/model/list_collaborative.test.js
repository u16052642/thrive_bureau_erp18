...
test("Can undo a command before a INSERT_THRIVE_LIST", async () => {
...
test("remove and update a domain of a list concurrently", async () => {
    insertList(alice, "1");
    await network.concurrent(() => {
        alice.dispatch("REMOVE_THRIVE_LIST", {
            listId: "1",
        });
        bob.dispatch("UPDATE_THRIVE_LIST_DOMAIN", {
            listId: "1",
            domain: [["foo", "in", [55]]],
        });
    });
...
test("remove and update a sorting of a list concurrently", async () => {
    ...
    await network.concurrent(() => {
        ...
        bob.dispatch("UPDATE_THRIVE_LIST", {
            listId: "1",
            list: {
                ...listDefinition,
                searchParams: {
...
test("Duplicate and remove list at the same time concurrently", async () => {
    ...
    await network.concurrent(() => {
        ...
        alice.dispatch("DUPLICATE_THRIVE_LIST", {
            listId: "1",
            newListId: "2",
        });
    });
...
test("Duplicate list concurrently", async () => {
    insertList(alice, "1");
    await network.concurrent(() => {
        bob.dispatch("DUPLICATE_THRIVE_LIST", {
            listId: "1",
            newListId: "2",
        });
        alice.dispatch("DUPLICATE_THRIVE_LIST", {
