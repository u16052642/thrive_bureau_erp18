...
test("THRIVE.FILTER.VALUE", async function () {
    ...
    for (const formula of [
        ...
        expect(composer.currentContent).toBe('=THRIVE.FILTER.VALUE("filter 1"');
...
test("escape double quotes in filter name", async function () {
    ...
    composer.startEdition("=THRIVE.FILTER.VALUE(");
    const autoComplete = composer.autocompleteProvider;
