...
current_repo="$(git remote get-url $current_remote | sed 's/.*github.com[\/:]//' | sed 's/\/thrive.git//')"
read -p "Enter repo [${current_repo}]: " REPO
REPO="https://github.com/${REPO:-$current_repo}/thrive.git"
...
echo "${VERSION_IOTBOX}" > "${MOUNT_POINT}"/var/thrive/iotbox_version
