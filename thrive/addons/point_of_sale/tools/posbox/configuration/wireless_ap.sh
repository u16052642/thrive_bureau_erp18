...
current_iotbox_version=$(cat "/var/thrive/iotbox_version")
...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ "${ESSID}" ] && [ "${PASSWORD}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		service thrive restart # As this file is executed on boot, this line is responsible for restarting thrive service on reboot
	fi
# wired
else
	killall nginx
