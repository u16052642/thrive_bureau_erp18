...
class TestEnv(IndustryCase):
    ...
    def test_knowledge_article_links_use_latest(self):
        pattern = r'https?://www\.thrive\.com/documentation/(?!latest/)(\d+\.\d+|master)/'
        for module in self.installed_modules:
            knowledge = self.env['ir.model.data'].search(
                [('model', '=', 'knowledge.article'), ('module', '=', module)], limit=1
            )
            ...
            self.assertFalse(
                matches,
                "Found links to Thrive documentation using version-specific URLs in module '%s' knowledge article: %s. Please use '/latest/' instead."
                % (module, matches),
            )
