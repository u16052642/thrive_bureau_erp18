...
export function stateToUrl(state) {
    ...
    const start_url = isScopedApp() ? "scoped_app" : "thrive";
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/thrive") &&
            (["/web", "/thrive"].includes(url.pathname) || url.pathname.startsWith("/thrive/")) &&
            ev.target.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/thrive") && url.hash) {
