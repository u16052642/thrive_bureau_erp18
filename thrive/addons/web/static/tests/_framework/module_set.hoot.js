...
import { describe, dryRun, globals, start, stop } from "@thrive/hoot";
import { Deferred, watchKeys, watchListeners } from "@thrive/hoot-mock";
...
const defineModuleSet = async (entryPoints, additionalAddons) => {
    ...
    if (additionalAddons.has("*")) {
        ...
        if (addons.has("spreadsheet")) {
            /**
             * spreadsheet addons defines a module that does not starts with `@spreadsheet` but `@thrive` (`@thrive/o-spreadsheet)
             * To ensure that this module is loaded, we have to include `thrive` in the dependencies
             */
            addons.add("thrive");
        }
...
class ModuleSetLoader extends loader.constructor {
    ...
    constructor(moduleSet) {
        ...
        thrive.define = this.define.bind(this);
        thrive.loader = this;
    }
    ...
    setup() {
        ...
        this.cleanups.push(
            watchKeys(window.thrive, WHITE_LISTED_KEYS),
...
const CSRF_TOKEN = thrive.csrf_token;
...
const R_DEFAULT_MODULE = /^@thrive\/(owl|hoot)/;
