import { expect, getFixture, test } from "@thrive/hoot";
import {
    contains,
    defineModels,
    fieldInput,
...
import { queryAllTexts, queryFirst } from "@thrive/hoot-dom";
...
test("in form view", async () => {
    Contact._records = [{ id: 1, email: "john.doe@thrivebureau.com" }];
    await mountView({
        type: "form",
        resModel: "contact",
        resId: 1,
    ...
    expect(`.o_field_email input[type="email"]`).toHaveValue("john.doe@thrivebureau.com");
    expect(`.o_field_email a`).toHaveCount(1);
    expect(`.o_field_email a`).toHaveAttribute("href", "mailto:john.doe@thrivebureau.com");
    expect(`.o_field_email a`).toHaveAttribute("target", "_blank");
    await fieldInput("email").edit("new@thrivebureau.com");
    expect(`.o_field_email input[type="email"]`).toHaveValue("new@thrivebureau.com");
});

test("in editable list view", async () => {
    Contact._records = [
        { id: 1, email: "john.doe@thrivebureau.com" },
        { id: 2, email: "jane.doe@thrivebureau.com" },
    ];
    await mountView({
        type: "list",
        resModel: "contact",
    ...
    expect(queryAllTexts(`tbody td:not(.o_list_record_selector) a`)).toEqual([
        "john.doe@thrivebureau.com",
        "jane.doe@thrivebureau.com",
    ]);
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:john.doe@thrivebureau.com");
    ...
    expect(queryAllTexts(`tbody td:not(.o_list_record_selector) a`)).toEqual([
        "new@thrivebureau.com",
        "jane.doe@thrivebureau.com",
    ]);
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:new@thrivebureau.com");
});
