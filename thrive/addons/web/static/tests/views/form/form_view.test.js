import { after, before, expect, test } from "@thrive/hoot";
import {
    clear,
    click,
    hover,
...
import { Deferred, animationFrame, mockTimeZone, mockTouch, runAllTimers } from "@thrive/hoot-mock";
import {
    Component,
    EventBus,
    onMounted,
...
test(`form view is not broken if save operation fails`, async () => {
    ...
    await animationFrame();
    expect(`.o_dialog`).toHaveCount(1);
    expect.verifyErrors(["RPC_ERROR: Thrive Bureau ERP server Error"]);
