import { after, beforeEach, describe, expect, getFixture, test } from "@thrive/hoot";
import { click, edit, queryAllProperties, queryAllTexts, queryFirst } from "@thrive/hoot-dom";
import { animationFrame, Deferred, mockSendBeacon, runAllTimers } from "@thrive/hoot-mock";
import {
    clickSave,
    defineActions,
    defineModels,
...
test("settings views does not write the id on the url", async () => {
    ...
    await getService("action").doAction(1);
    await runAllTimers();
    expect(browser.location.pathname).toBe("/thrive/settings");
    expect(".o_field_boolean input").not.toHaveProperty("disabled");
    await click(".o_field_boolean input");
    await animationFrame();
...
test("Open settings from url, with setting id anchor", async () => {
    ...
    redirect("/thrive/settings#setting_id");
