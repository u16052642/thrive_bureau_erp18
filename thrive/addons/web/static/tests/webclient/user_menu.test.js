...
import { click, queryAllAttributes, queryAllProperties, queryAllTexts } from "@thrive/hoot-dom";
...
import { thriveAccountItem, preferencesItem } from "@web/webclient/user_menu/user_menu_items";
...
test("click on thrive account item", async () => {
    patchWithCleanup(browser, {
        open: (url) => expect.step(`open ${url}`),
    });
    userMenuRegistry.add("thrive_account", thriveAccountItem);
    await mountWithCleanup(UserMenu);
    onRpc("/web/session/account", () => "https://account-url.com");
    stepAllNetworkCalls();
    await contains("button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveCount(1);
    expect(".o-dropdown--menu .dropdown-item").toHaveText("My thrivebureau.com account");
