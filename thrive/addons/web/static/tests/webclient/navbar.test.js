import { beforeEach, destroy, expect, test } from "@thrive/hoot";
import { queryAll, queryAllAttributes, queryAllTexts, resize } from "@thrive/hoot-dom";
...
import { Component, onRendered, xml } from "@thrive/owl";
...
test.tags("desktop")("href attribute on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/thrive/action-339");
});
...
test.tags("desktop")("href attribute with path on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/thrive/my-path");
});
