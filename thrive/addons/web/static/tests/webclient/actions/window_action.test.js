...
import { click, queryAllTexts, waitFor } from "@thrive/hoot-dom";
...
test.tags("desktop")("A new form view can be reloaded after a failed one", async () => {
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/thrive/action-3/1");
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/thrive/action-3/2");
