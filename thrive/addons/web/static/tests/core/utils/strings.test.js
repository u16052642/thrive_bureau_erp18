...
test("escape", () => {
    ...
    expect(escape(`<a href="https://www.thrivebureau.com">thrive<a>`)).toBe(
        `&lt;a href=&quot;https://www.thrivebureau.com&quot;&gt;thrive&lt;a&gt;`
    );
    expect(escape(`<a href='https://www.thrivebureau.com'>thrive<a>`)).toBe(
        `&lt;a href=&#x27;https://www.thrivebureau.com&#x27;&gt;thrive&lt;a&gt;`
    );
    expect(escape("<a href='https://www.thrivebureau.com'>Thrive`s website<a>")).toBe(
...
test("escapeRegExp", () => {
    ...
    expect(escapeRegExp("[wowl]")).toBe("\\[wowl\\]");
    expect(escapeRegExp("[wowl.thrive]")).toBe("\\[wowl\\.thrive\\]");
    expect(escapeRegExp("^thrive.define([.]*)$")).toBe("\\^thrive\\.define\\(\\[\\.\\]\\*\\)\\$");
    expect(escapeRegExp("[.*+?^${}()|[]\\")).toBe("\\[\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\]\\\\");
});
...
test("isEmail", () => {
    ...
    expect(isEmail("test@thrive")).toBe(false);
    expect(isEmail("test@thrive@thrivebureau.com")).toBe(false);
    expect(isEmail("te st@thrivebureau.com")).toBe(false);

    expect(isEmail("test@thrivebureau.com")).toBe(true);
});
