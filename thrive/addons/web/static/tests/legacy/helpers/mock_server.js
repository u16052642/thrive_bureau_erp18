/** @thrive-module alias=@web/../tests/helpers/mock_server default=false */
...
export function makeServerError({
    ...
    return makeErrorFromResponse({
        ...
        data: {
            name: errorName || `thrive.exceptions.${type || "UserError"}`,
            debug: "traceback",
            arguments: [],
            context: context || {},
            subType,
    ...
    mockLoadAction({ action_id } = {}) {
        ...
        if (!action) {
            throw makeServerError({
                errorName: "thrive.addons.web.controllers.action.MissingActionError",
