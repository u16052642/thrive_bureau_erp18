...
from thrive import http, modules
from thrive.exceptions import AccessError
from thrive.http import request
from thrive.tools import file_open, file_path, image_process
...
class WebManifest(http.Controller):
        ...
        for module in module_ids:
            ...
            if data:
                shortcuts.append({
                    ...
                    'url': '/thrive?menu_id=%s' % data.mapped('res_id')[0],
    ...
    def service_worker(self):
        response = request.make_response(
            ...
            [
                ...
                ('Service-Worker-Allowed', '/thrive'),
            ]
        )
        return response

    def _get_service_worker_content(self):
        """ Returns a ServiceWorker javascript file scoped for the backend (aka. '/thrive')
        """
    ...
    @http.route('/thrive/offline', type='http', auth='public', methods=['GET'])
    def offline(self):
    ...
    def scoped_app(self, app_id, path='', app_name=''):
        ...
        scoped_app_values = {
            'app_id': app_id,
            'apple_touch_icon': '/web/static/img/thrive-icon-ios.png',
            'app_name': app_name,
            'path': path,
    ...
    def scoped_app_manifest(self, app_id, path, app_name=''):
        """ Returns a WebManifest dedicated to the scope of the given app. A custom scope and start
            url are set to make sure no other installed PWA can overlap the scope (e.g. /thrive)
        """
        path = unquote(path)
        app_name = unquote(app_name) if app_name else self._get_scoped_app_name(app_id)
        webmanifest = {
