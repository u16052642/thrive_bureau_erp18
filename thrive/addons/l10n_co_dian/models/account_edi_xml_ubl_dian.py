...
from thrive.addons.l10n_co_dian import xml_utils
from thrive.tools import cleanup_xml_node, float_repr
from thrive.addons.l10n_co_edi.models.res_partner import FINAL_CONSUMER_VAT
from thrive.addons.l10n_co_edi.models.account_invoice import L10N_CO_EDI_TYPE
    ...
    def _dian_tax_totals(self, move, taxes_vals, withholding):
        ...
        for grouping_key, vals in filtered_tax_details.items():
            ...
            if tax_co_type == '34':
                ...
                # In Thrive, we use fixed taxes and a field for the volume of the product: l10n_co_edi_ref_nominal_tax
        ...
        if '05' in tax_total_dict and move.l10n_co_edi_is_support_document and withholding:
            ...
            # Hence, the tax "15% RteVAT 19%" is encoded as a -2.85% tax in Thrive
            if 'tax_details_per_record' in taxes_vals:
