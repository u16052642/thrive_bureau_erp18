...
import { animationFrame, advanceTime } from "@thrive/hoot-mock";
...
class PeerTest {
    ...
    async focus() {
        return this.plugins["collaboration_thrive"].joinPeerToPeer();
    }
    ...
    async writeToServer() {
        ...
        const lastId = this.plugins.collaboration_thrive.getLastHistoryStepId(
        ...
        for (const peer of Object.values(this.peers)) {
            ...
            peer.onlineMutex.exec(async () => {
                return peer.plugins.collaboration_thrive.onServerLastIdUpdate(String(lastId));
            });
        }
    }
    async setOnline() {
...
class Wysiwygs extends Component {
    ...
    onLoad(peerId, editor) {
        ...
        editor.startPlugins = () => {
            ...
            patch(plugins["collaboration_thrive"], {
                getMetadata() {
...
describe("Stale detection & recovery", () => {
    describe("detect stale while unfocused", () => {
        test("should do nothing until focus", async () => {
            ...
            expect(peers.p1.plugins.collaboration_thrive.isDocumentStale).toBe(false, {
            ...
            expect(peers.p2.plugins.collaboration_thrive.isDocumentStale).toBe(false, {
            ...
            expect(peers.p3.plugins.collaboration_thrive.isDocumentStale).toBe(true, {
            ...
            expect(peers.p3.plugins.collaboration_thrive.isDocumentStale).toBe(false, {
    ...
    describe("detect stale while focused", () => {
        describe("recover from missing steps", () => {
            test("should recover from missing steps", async () => {
                ...
                const p3Spies = makeSpies(peers.p3.plugins.collaboration_thrive, [
                ...
                expect(peers.p1.plugins.collaboration_thrive.historyShareId).toBe(
                    peers.p2.plugins.collaboration_thrive.historyShareId,
                    {
                        message: "p1 and p2 should have the same historyShareId",
                    }
                );
                expect(peers.p1.plugins.collaboration_thrive.historyShareId).toBe(
                    peers.p3.plugins.collaboration_thrive.historyShareId,
                    {
        ...
        describe("recover from snapshot", () => {
            test("should wait for all peer to recover from snapshot", async () => {
                ...
                const p2Spies = makeSpies(peers.p2.plugins.collaboration_thrive, [
                    "recoverFromStaleDocument",
                    "resetFromServerAndResyncWithPeers",
                    "processMissingSteps",
                    "applySnapshot",
...
describe("Disconnect & reconnect", () => {
    test("should sync history when disconnecting and reconnecting to internet", async () => {
        ...
        const p1PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p2.plugins.collaboration_thrive, {
        ...
        const p2PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p1.plugins.collaboration_thrive, {
                async processMissingSteps() {
                    super.processMissingSteps(...arguments);
                    resolve();
                },
...
describe("History steps Ids", () => {
    ...
    test("should clear history step ids when resetting from server", async () => {
        ...
        expect(peers.p2.plugins.collaboration_thrive.isDocumentStale).toBe(true, {
