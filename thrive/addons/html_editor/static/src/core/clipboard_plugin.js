...
export class ClipboardPlugin extends Plugin {
    ...
    onCopy(ev) {
        ...
        const thriveText = selection.textContent();
        ev.clipboardData.setData("text/plain", thriveText);
        ev.clipboardData.setData("text/html", thriveHtml);
        ev.clipboardData.setData("application/vnd.thrive.thrive-editor", thriveHtml);
    }
    ...
    onPaste(ev) {
        ...
        this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
            this.handlePasteThriveEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteThriveEditorHtml(clipboardData) {
        const thriveEditorHtml = clipboardData.getData("application/vnd.thrive.thrive-editor");
        if (thriveEditorHtml) {
    ...
    onDragStart(ev) {
        if (ev.target.nodeName === "IMG") {
            ...
            ev.dataTransfer.setData(
                "application/vnd.thrive.thrive-editor-node",
    ...
    async onDrop(ev) {
        ...
        const imageNodeHTML = ev.dataTransfer.getData("application/vnd.thrive.thrive-editor-node");
        const image =
            imageNodeHTML &&
            this.dragImage &&
...
// @phoenix @todo: move to Thrive Bureau ERP plugin?
/**
