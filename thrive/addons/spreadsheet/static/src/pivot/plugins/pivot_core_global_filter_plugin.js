...
export class PivotCoreGlobalFilterPlugin extends ThriveCorePlugin {
        ...
        globalFiltersFieldMatchers["pivot"] = {
            getIds: () =>
                this.getters
                    ...
                    .filter(
                        (id) =>
                            this.getters.getPivotCoreDefinition(id).type === "THRIVE" &&
                            id in this.pivots
                    ),
            ...
            getModel: (pivotId) => {
                ...
                return pivot.type === "THRIVE" && pivot.model;
            },
        };
    }
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "DUPLICATE_PIVOT": {
                ...
                if(pivotDefinition.type !== "THRIVE") {
                    break;
                }
    ...
    export(data) {
        for (const id in this.pivots) {
            ...
            data.pivots[id].fieldMatching =
                pivot.type === "THRIVE" ? this.pivots[id].fieldMatching : {};
        }
    }
}
