...
import { EvaluationError, helpers } from "@thrive/o-spreadsheet";
 ...
 * @typedef {import("@thrive/o-spreadsheet").Granularity} Granularity
 * */
...
export const THRIVE_AGGREGATORS = {};

for (const type in AGGREGATORS_BY_FIELD_TYPE) {
    THRIVE_AGGREGATORS[type] = {};
    for (const aggregator of AGGREGATORS_BY_FIELD_TYPE[type]) {
        THRIVE_AGGREGATORS[type][aggregator] = AGGREGATOR_NAMES[aggregator];
    }
}
...
/**
 * @typedef {import("@spreadsheet").ThriveField} ThriveField
 */

/**
 * Parses the positional char (#), the field and operator string of pivot group.
 * e.g. "create_date:month"
 * @param {Record<string, ThriveField | undefined>} allFields
 * @param {string} groupFieldString
 * @returns {{field: ThriveField, granularity: Granularity, isPositional: boolean, dimensionWithGranularity: string}}
 */
