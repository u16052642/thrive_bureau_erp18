...
import { NO_RECORD_AT_THIS_POSITION, ThrivePivotModel } from "./pivot_model";
import { EvaluationError, PivotRuntimeDefinition, registries, helpers } from "@thrive/o-spreadsheet";
import { LOADING_ERROR } from "@spreadsheet/data_sources/data_source";
import { omit } from "@web/core/utils/objects";
import { ThrivePivotLoader } from "./thrive_pivot_loader";
...
/**
 * @typedef {import("@thrive/o-spreadsheet").FunctionResultObject} FunctionResultObject
 * @typedef {import("@thrive/o-spreadsheet").PivotMeasure} PivotMeasure
 * @typedef {import("@thrive/o-spreadsheet").PivotDomain} PivotDomain
 * @typedef {import("@thrive/o-spreadsheet").PivotDimension} PivotDimension
 * @typedef {import("@spreadsheet").WebPivotModelParams} WebPivotModelParams
 * @typedef {import("@spreadsheet").ThrivePivot<ThrivePivotRuntimeDefinition>} IPivot
 * @typedef {import("@spreadsheet").ThriveFields} ThriveFields
 * @typedef {import("@spreadsheet").ThrivePivotCoreDefinition} ThrivePivotCoreDefinition
 * @typedef {import("@spreadsheet").SortedColumn} SortedColumn
 * @typedef {import("@spreadsheet").ThriveGetters} ThriveGetters
 ...
 * @implements {IPivot}
 */
export class ThrivePivot {
    /**
     *
     * @override
     * @param {Object} services Services (see DataSource)
     * @param {Object} params
     * @param {ThrivePivotCoreDefinition} params.definition
     * @param {ThriveGetters} params.getters
     */
    constructor(services, { definition, getters }) {
        /** @type {"THRIVE"} */
        this.type = "THRIVE";

        /** @type {ThrivePivotCoreDefinition} @protected */
        this.coreDefinition = definition;

        this.needsReevaluation = false;

        /** @type {ThrivePivotRuntimeDefinition | undefined} @protected */
        this.runtimeDefinition = undefined;

        /** @type {ThrivePivotModel | undefined} @protected */
        this.model = undefined;

        /** @type {ThriveGetters} @protected */
        this.getters = getters;

        /** @protected */
        this.loader = new ThrivePivotLoader(services.thriveDataProvider, this._load.bind(this));

        /** @type {ThriveFields | undefined} @protected */
        this._fields = undefined;

        /** @protected @type {ThriveDataProvider}*/
        this.thriveDataProvider = services.thriveDataProvider;
    ...
    /**
     * @param {ThrivePivotCoreDefinition} nextDefinition
     */
    onDefinitionChange(nextDefinition) {
        ...
        if (
            ...
            if (this.isMeasuresTheSameForData(actualDefinition.measures, nextDefinition.measures)) {
                this.coreDefinition = nextDefinition;
                const runtimeDefinition = new ThrivePivotRuntimeDefinition(
                    this.coreDefinition,
                    this.getFields()
                );
    ...
    async createModelAndDefinition() {
        ...
        const definition = new ThrivePivotRuntimeDefinition(this.coreDefinition, this.getFields());
        const model = new ThrivePivotModel(
            { _t },
            {
            ...
            {
                orm: this.thriveDataProvider.orm,
                serverData: this.thriveDataProvider.serverData,
            }
        );
    ...
    /**
     * @param {import("@thrive/o-spreadsheet").Maybe<FunctionResultObject>[]} args
     *
     * @returns {PivotDomain}
     */
...
export class ThrivePivotRuntimeDefinition extends PivotRuntimeDefinition {
    /**
     * @param {ThrivePivotCoreDefinition} definition
     * @param {ThriveFields} fields
     */
...
supportedPivotPositionalFormulaRegistry.add("THRIVE", true);
