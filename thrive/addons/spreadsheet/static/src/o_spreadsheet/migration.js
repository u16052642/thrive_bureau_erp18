...
export const THRIVE_VERSION = 12;
...
const MAP_FN_NAMES_V10 = {
    "THRIVE.PIVOT": "PIVOT.VALUE",
    "THRIVE.PIVOT.HEADER": "PIVOT.HEADER",
    "THRIVE.PIVOT.TABLE": "PIVOT",
};

const dmyRegex = /^([0|1|2|3][1-9])\/(0[1-9]|1[0-2])\/(\d{4})$/i;

migrationStepRegistry.add("thrive_migration", {
    versionFrom: "16.1",
    migrate(data) {
        return migrateThriveData(data);
    },
});

function migrateThriveData(data) {
    const version = data.thriveVersion || 0;
    if (version < 1) {
...
/**
 * Migrate the pivot data to add the type, by default "THRIVE". And replace the
...
function migrate11to12(data) {
    // remove the calls to THRIVE.PIVOT.POSITION and replace
    ...
    for (const sheet of data.sheets || []) {
        for (const xc in sheet.cells || []) {
            ...
            if (
                ...
                cell.content.includes("THRIVE.PIVOT.POSITION")
            ) {
                const tokens = tokenize(cell.content);
                /* given that thrive.pivot.position is automatically set, we know that:
                1) it is always on the form of THRIVE.PIVOT.POSITION(1, ...)
                2) it is always preceded by a dimension of a pivot or header, inside another pivot formula
                3) there is only one thrive.pivot.position per cell
                4) thrive.pivot.position can only exist after the 3rd token and needs at least 7 tokens to be valid*/
                for (let i = 2; i < tokens.length - 7; i++) {
                    const token = tokens[i];
                    if (
                        token.type === "SYMBOL" &&
                        token.value.toUpperCase() === "THRIVE.PIVOT.POSITION"
                    ) {
                        const order = tokens[i + 6];
                        tokens[i - 2].value = '"#' + tokens[i - 2].value.slice(1); // "dimension" becomes "#dimension"
                        tokens.splice(i, 7); // remove "THRIVE.PIVOT.POSITION", "(", "1", ",", "dimension", ", ", order
                        // tokens[i-1] is the comma before thrive.pivot.position
                        tokens[i] = order;
                        cell.content = tokensToString(tokens);
                    }
                }
...
export class ThriveVersion extends ThriveCorePlugin {
