...
import { animationFrame, mockDate, mockTimeZone } from "@thrive/hoot-mock";

import { DispatchResult, Model, helpers, tokenize } from "@thrive/o-spreadsheet";
...
test("THRIVE.FILTER.VALUE text filter", async function () {
    const model = await createModelWithDataSource();
...
test("THRIVE.FILTER.VALUE date filter", async function () {
    const model = await createModelWithDataSource();
...
test("THRIVE.FILTER.VALUE date from/to without values", async function () {
    const model = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(model, "A1", `=THRIVE.FILTER.VALUE("Date Filter")`);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    expect(getEvaluatedCell(model, "B1").value).toBe("");
});

test("THRIVE.FILTER.VALUE date from/to with only from defined", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=THRIVE.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("THRIVE.FILTER.VALUE date from/to with only to defined", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=THRIVE.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("THRIVE.FILTER.VALUE date from/to with from and to defined", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=THRIVE.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("THRIVE.FILTER.VALUE relation filter", async function () {
    const model = await createModelWithDataSource({
...
test("THRIVE.FILTER.VALUE with escaped quotes in the filter label", async function () {
    const model = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "text",
    ...
    setCellContent(model, "A1", '=THRIVE.FILTER.VALUE("my \\"special\\" filter")');
    expect(getCellValue(model, "A1")).toBe("Jean-Jacques");
});

test("THRIVE.FILTER.VALUE formulas are updated when filter label is changed", async function () {
    const model = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
...
test("field matching is removed when an Thrive chart is deleted", async function () {
    ...
    expect(model.getters.getThriveChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
    expect(() => model.getters.getThriveChartFieldMatching(chartId, filter.id)).toThrow(undefined, {
