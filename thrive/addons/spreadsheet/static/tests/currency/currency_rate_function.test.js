...
test("rate formula at a given company", async () => {
    ...
    setCellContent(model, "A1", `=THRIVE.CURRENCY.RATE("EUR","USD",, 1)`);
    setCellContent(model, "A2", `=THRIVE.CURRENCY.RATE("EUR","USD",, 2)`);
    await waitForDataLoaded(model);
    expect.verifySteps(["rate fetched"]);
...
test("invalid company id", async () => {
    ...
    setCellContent(model, "A1", `=THRIVE.CURRENCY.RATE("EUR","USD",, 45)`);
    await waitForDataLoaded(model);
    expect(getCellValue(model, "A1")).toBe("#ERROR");
