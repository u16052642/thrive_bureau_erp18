...
test("List export", async () => {
    ...
    expect(getCellFormula(model, "A1")).toBe(`=THRIVE.LIST.HEADER(1,"foo")`);
    expect(getCellFormula(model, "B1")).toBe(`=THRIVE.LIST.HEADER(1,"bar")`);
    expect(getCellFormula(model, "C1")).toBe(`=THRIVE.LIST.HEADER(1,"date")`);
    expect(getCellFormula(model, "D1")).toBe(`=THRIVE.LIST.HEADER(1,"product_id")`);
    expect(getCellFormula(model, "A2")).toBe(`=THRIVE.LIST(1,1,"foo")`);
    expect(getCellFormula(model, "B2")).toBe(`=THRIVE.LIST(1,1,"bar")`);
    expect(getCellFormula(model, "C2")).toBe(`=THRIVE.LIST(1,1,"date")`);
    expect(getCellFormula(model, "D2")).toBe(`=THRIVE.LIST(1,1,"product_id")`);
    expect(getCellFormula(model, "A3")).toBe(`=THRIVE.LIST(1,2,"foo")`);
    expect(getCellFormula(model, "A11")).toBe(`=THRIVE.LIST(1,10,"foo")`);
...
test("Can remove a list with undo after editing a cell", async function () {
    const { model } = await createSpreadsheetWithList();
    expect(getCellContent(model, "B1").startsWith("=THRIVE.LIST.HEADER")).toBe(true);
...
test("Undo/Redo for RENAME_THRIVE_LIST", async function () {
    const { model } = await createSpreadsheetWithList();
...
test("can update a list", async () => {
    ...
    model.dispatch("UPDATE_THRIVE_LIST", {
        listId,
        list: newListDef,
    });
    await waitForDataLoaded(model);
...
test("editing the sorting of a list of that does not exist should throw an error", async () => {
    const { model } = await createSpreadsheetWithList();
    const result = model.dispatch("UPDATE_THRIVE_LIST", {
        listId: "invalid",
        list: undefined,
    });
    expect(result.reasons).toEqual([CommandResult.ListIdNotFound]);
...
test("edited domain is exported", async () => {
    ...
    const result = model.dispatch("UPDATE_THRIVE_LIST_DOMAIN", {
        listId: "invalid",
        domain: [],
    });
    expect(result.reasons).toEqual([CommandResult.ListIdNotFound]);
        ...
        sheets: [
            {
                id: "sheet1",
                cells: {
                    A1: { content: '=THRIVE.LIST(1, 1, "foo")' }, // in the definition
                    A2: { content: '=THRIVE.LIST(1, 1, "product_id")' }, // not in the definition
                    A3: { content: '=THRIVE.LIST(1, 1, "invalid_field")' },
                },
            },
        ],
        lists: {
        ...
        sheets: [
            {
                id: "sheet1",
                cells: {
                    A1: { content: '=THRIVE.LIST(1, 11, "foo")' },
                    A2: { content: '=THRIVE.LIST(1, A3, "foo")' },
                    A3: { content: "111" },
                },
            },
        ],
...
test("list with both a monetary field and the related currency field 1", async function () {
    ...
    setCellContent(model, "A1", '=THRIVE.LIST(1, 1, "pognon")');
    setCellContent(model, "A2", '=THRIVE.LIST(1, 1, "currency_id")');
    await animationFrame();
...
test("Spec of web_search_read is minimal", async function () {
    ...
    setCellContent(model, "A1", '=THRIVE.LIST(1, 1, "pognon")');
    setCellContent(model, "A2", '=THRIVE.LIST(1, 1, "currency_id")');
    setCellContent(model, "A3", '=THRIVE.LIST(1, 1, "foo")');
    await waitForDataLoaded(model);
    expect.verifySteps(["web_search_read"]);
});
...
test("Can duplicate a list", async () => {
    ...
    model.dispatch("DUPLICATE_THRIVE_LIST", { listId, newListId: "2" });
...
test("Cannot duplicate unknown list", async () => {
    const { model } = await createSpreadsheetWithList();
    const result = model.dispatch("DUPLICATE_THRIVE_LIST", {
        listId: "hello",
...
test("isListUnused getter", async () => {
    ...
    setCellContent(model, "A1", '=THRIVE.LIST.HEADER(1, "foo")');
    expect(model.getters.isListUnused("1")).toBe(false);

    setCellContent(model, "A1", '=THRIVE.LIST.HEADER(A2, "foo")');
    expect(model.getters.isListUnused("1")).toBe(true);
...
test("INSERT_THRIVE_LIST_WITH_TABLE adds a table that maches the list dimension", async function () {
    const { model } = await createSpreadsheetWithList({
        linesNumber: 4,
    });
    const sheetId = model.getters.getActiveSheetId();
    ...
    model.dispatch("INSERT_THRIVE_LIST_WITH_TABLE", {
        sheetId,
        col,
        row,
