...
import { THRIVE_VERSION } from "@spreadsheet/o_spreadsheet/migration";
import { Model, load } from "@thrive/o-spreadsheet";
...
test("Thrive formulas are migrated", () => {
    const data = {
        version: 16,
        sheets: [
            {
    ...
    expect(migratedData.sheets[0].cells.A3.content).toBe(`=THRIVE.FILTER.VALUE("1")`);
    expect(migratedData.sheets[0].cells.A4.content).toBe(`=THRIVE.LIST("1")`);
    expect(migratedData.sheets[0].cells.A5.content).toBe(`=THRIVE.LIST.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A6.content).toBe(`=THRIVE.PIVOT.POSITION("1")`);
...
test("Pivot formulas are migrated from 9 to 10", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=THRIVE.PIVOT("1")` },
                    A2: { content: `=THRIVE.PIVOT.HEADER("1")` },
                    A3: { content: `=THRIVE.PIVOT.POSITION("1")` },
                    A4: { content: `=THRIVE.PIVOT.TABLE("1")` },
                    A5: { content: `=thrive.pivot("1")` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1.content).toBe(`=PIVOT.VALUE("1")`);
    expect(migratedData.sheets[0].cells.A2.content).toBe(`=PIVOT.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A3.content).toBe(`=THRIVE.PIVOT.POSITION("1")`);
...
test("Pivot formulas using pivot positions are migrated (11 to 12)", () => {
        ...
        sheets: [
            {
                cells: {
                    A1: {
                        content: `=-PIVOT.VALUE("1","balance","account_id",THRIVE.PIVOT.POSITION("1","account_id",12),"date:quarter","4/"&THRIVE.FILTER.VALUE("Year"))`,
                    },
                    A2: {
                        content: `=PIVOT.HEADER("1","account_id",THRIVE.PIVOT.POSITION("1","account_id",14))`,
                    },
                    A3: { content: `=THRIVE.PIVOT.POSITION("1","account_id",14)` },
                    A4: { content: `=THRIVE.PIVOT.POSITION("1",14)` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1.content).toBe(
        `=-PIVOT.VALUE("1","balance","#account_id",12,"date:quarter","4/"&THRIVE.FILTER.VALUE("Year"))`
    );
    ...
    expect(migratedData.sheets[0].cells.A3.content).toBe(
        `=THRIVE.PIVOT.POSITION("1","account_id",14)`
    );
    expect(migratedData.sheets[0].cells.A4.content).toBe(`=THRIVE.PIVOT.POSITION("1",14)`);
});

test("Thrive version is exported", () => {
    const model = new Model();
    expect(model.exportData().thriveVersion).toBe(THRIVE_VERSION);
});
