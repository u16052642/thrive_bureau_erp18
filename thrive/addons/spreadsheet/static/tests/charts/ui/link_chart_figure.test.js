...
import { expect, test, beforeEach } from "@thrive/hoot";
...
test("icon external link isn't on the chart when its not linked to an thrive menu", async function () {
    ...
    expect(thriveMenu).toBe(undefined, { message: "No menu linked with the chart" });

    const externalRefIcon = fixture.querySelector(".o-chart-external-link");
    expect(externalRefIcon).toBe(null);
});

test("icon external link is on the chart when its linked to an thrive menu", async function () {
    const model = await createModelWithDataSource({
        serverData,
    });
    ...
    expect(chartMenu.id).toBe(1, { message: "Thrive menu is linked to chart" });
    await animationFrame();
    expect(".o-chart-external-link").toHaveCount(1);
});

test("icon external link is not on the chart when its linked to a wrong thrive menu", async function () {
    const model = await createModelWithDataSource({
        serverData,
    });
    await mountSpreadsheet(model);
...
test("click on icon external link on chart redirect to the thrive menu", async function () {
    ...
    const chartMenu = model.getters.getChartThriveMenu(chartId);
    expect(chartMenu.id).toBe(2, { message: "Thrive menu is linked to chart" });
    await animationFrame();
...
test("Click on chart in dashboard mode redirect to the thrive menu", async function () {
    const doActionStep = "doAction";
    mockActionService(doActionStep);
    const model = await createModelWithDataSource({
        serverData,
