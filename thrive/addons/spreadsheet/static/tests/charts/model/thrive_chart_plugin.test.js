...
test("Can add an Thrive Bar chart", async () => {
    ...
    expect(chart instanceof ThriveBarChart).toBe(true);
...
test("Thrive bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "thrive_bar",
...
test("Thrive pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "thrive_pie",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Thrive line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "thrive_line",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Can import/export an Thrive chart", async () => {
    ...
    expect(figure.data.type).toBe("thrive_line");
...
test("Can undo/redo an Thrive chart creation", async () => {
    const model = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "thrive_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("Can copy/paste Thrive chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "thrive_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof ThriveChart).toBe(true);
...
test("Can cut/paste Thrive chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "thrive_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof ThriveChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates Thrive chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "thrive_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load thrive chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert thrive chart from a different model", async () => {
    const model = await createModelWithDataSource();
...
test("Thrive chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "thrive_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove thrive chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "thrive_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getThriveChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getThriveChartIds().length).toBe(0);
});

test("Thrive chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "thrive_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getThriveChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getThriveChartDisplayName(chartId)).toBe("(#1) Thrive Line Chart");
});
