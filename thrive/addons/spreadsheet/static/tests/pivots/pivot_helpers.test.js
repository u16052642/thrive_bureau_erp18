...
import { constants, tokenize, helpers } from "@thrive/o-spreadsheet";
...
describe("pivot_helpers", () => {
    test("Basic formula extractor", async function () {
        const formula = `=PIVOT.VALUE("1", "test") + THRIVE.LIST("2", "hello", "bla")`;
        ...
        expect(functionName).toBe("THRIVE.LIST");
    ...
    test("Number of formulas", async function () {
        const formula = `=PIVOT.VALUE("1", "test") + PIVOT.VALUE("2", "hello", "bla") + THRIVE.LIST("1", "bla")`;
