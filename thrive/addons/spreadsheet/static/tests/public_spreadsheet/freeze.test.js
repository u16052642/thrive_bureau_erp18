...
import { freezeThriveData } from "@spreadsheet/helpers/model";
import { ThrivePivot, ThrivePivotRuntimeDefinition } from "@spreadsheet/pivot/thrive_pivot";
...
test("thrive pivot functions are replaced with their value", async function () {
...
test("Pivot with a type different of THRIVE is not converted", async function () {
    // Add a pivot with a type different of THRIVE
...
test("thrive pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezeThriveData(model);
...
test("thrive charts are replaced with an image", async function () {
...
test("thrive links are replaced with their label", async function () {
    const view = {
        name: "an thrive view",
        viewType: "list",
        action: {
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: { content: "[menu_xml](thrive://ir_menu_xml_id/test_menu)" },
                    A2: { content: "[menu_id](thrive://ir_menu_id/12)" },
                    A3: { content: `[thrive_view](thrive://view/${JSON.stringify(view)})` },
                    A4: { content: "[external_link](https://thrivebureau.com)" },
    ...
    const frozenData = await freezeThriveData(model);
    expect(frozenData.sheets[0].cells.A1.content).toBe("menu_xml");
    expect(frozenData.sheets[0].cells.A2.content).toBe("menu_id");
    expect(frozenData.sheets[0].cells.A3.content).toBe("thrive_view");
    expect(frozenData.sheets[0].cells.A4.content).toBe("[external_link](https://thrivebureau.com)");
