...
test("click a web link", async () => {
    ...
    expect(urlRepresentation(cell.link, model.getters)).toBe("https://thrivebureau.com");
    openLink(cell.link, env);
    expect.verifySteps(["https://thrivebureau.com"]);
});
...
test("click a menu link [2]", async () => {
    ...
    setCellContent(model, "A1", `[a view](thrive://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("an thrive view");
    openLink(cell.link, env);
...
test("Click a link containing an action xml id", async () => {
    ...
    const model = new Model({}, { custom: { env } });
    setCellContent(model, "A1", `[an action link](thrive://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
