...
def filter_fields(data):
    ...
    if thrive_version < 5:
        ...
        for pivot in data["pivots"].values():
            if pivot.get("type", "THRIVE") == "THRIVE":
                model = pivot["model"]
...
def fields_in_spreadsheet(data):
    ...
    thrive_pivots = (pivot for pivot in data.get("pivots", dict()).values() if pivot.get("type", "THRIVE") == "THRIVE")
    all_fields = chain(
        extract_fields(list_fields, data.get("lists", dict()).values()).items(),
        extract_fields(pivot_fields, thrive_pivots).items(),
