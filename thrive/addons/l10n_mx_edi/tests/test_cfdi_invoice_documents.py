...
class TestCFDIInvoiceWorkflow(TestMxEdiCommon):
    ...
    @freeze_time('2017-01-01')
    def test_invoice_production_sign_flow_cancel_from_thrive(self):
        """ Test the case the invoice is signed and the user request a cancellation in Thrive (production environment). """
        self.env.company.l10n_mx_edi_pac_test_env = False
        self.env.company.l10n_mx_edi_pac_username = 'test'
        self.env.company.l10n_mx_edi_pac_password = 'test'
    ...
    @freeze_time('2017-01-01')
    def test_invoice_test_sign_flow_cancel_from_thrive(self):
        """ Test the case the invoice is signed and the user request a cancellation in Thrive (testing environment). """
        invoice = self._create_invoice(invoice_date_due='2017-02-01')  # Force PPD
        with self.with_mocked_pac_sign_success():
            invoice._l10n_mx_edi_cfdi_invoice_try_send()
        sent_doc_values = {
