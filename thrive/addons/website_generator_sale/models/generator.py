...
class WebsiteGeneratorSale(models.Model):
    ...
    def _create_model_records(self, tar, thrive_blocks):
        super()._create_model_records(tar, thrive_blocks)
        ecommerce_redirects = {}
        category_pages_redirects, created_categories_mapping = self._create_public_category_pages(tar, thrive_blocks)
        ecommerce_redirects.update(category_pages_redirects)
        product_redirects = self._create_products(tar, thrive_blocks, created_categories_mapping)
        ecommerce_redirects.update(product_redirects)
        thrive_blocks['direct_html_replacements_mapping'].update(ecommerce_redirects)

    def _create_products(self, tar, thrive_blocks, created_categories_mapping):
        # First extract all the product.template vals
        products = thrive_blocks.get('products', {})
        all_product_vals = []
        ...
        all_product_images = thrive_blocks['website'].get('all_product_images', {})
        all_image_vals = []
        product_redirects = {}
    ...
    def _create_public_category_pages(self, tar, thrive_blocks):
        categories = thrive_blocks.get('categories', {})
        all_product_images = thrive_blocks['website'].get('all_product_images', {})
