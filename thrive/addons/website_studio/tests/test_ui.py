...
class TestUi(thrive.tests.HttpCase):
    ...
    def test_listing_and_page_creation(self):
        ...
        self.start_tour("/thrive", 'website_studio_listing_and_page', login="admin", timeout=3600)
    ...
    def test_listing_without_page_creation(self):
        ...
        self.start_tour("/thrive", 'website_studio_listing_without_page', login="admin", watch=False, timeout=3600)
        created_pages = self.env["website.controller.page"].search([])
        self.assertEqual(len(created_pages), 1)
