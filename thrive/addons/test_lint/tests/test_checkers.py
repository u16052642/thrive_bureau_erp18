...
from thrive.tools.which import which
...
class TestGetTextLint(TestPylintChecks):
    def check(self, testtext):
        return super().check(testtext, "_thrive_checker_gettext", "gettext-placeholders")
...
class TestSqlLint(TestPylintChecks):
    def check(self, testtext):
        return super().check(testtext, "_thrive_checker_sql_injection", "sql-injection")
...
class TestI18nChecks(TestPylintChecks):
    def check(self, test_content):
        return super().check(
            test_content, "_thrive_checker_gettext", "gettext-variable,gettext-placeholders,gettext-repr"
        )
