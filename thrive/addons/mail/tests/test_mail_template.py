...
class TestMailTemplate(MailCommon):
    ...
    def test_mail_template_acl(self):
        ...
        for expression in allowed_qweb_expressions:
            ...
            with (patch('thrive.addons.base.models.ir_qweb.IrQWeb._render', side_effect=o_qweb_render) as qweb_render,
                patch('thrive.addons.base.models.ir_qweb.unsafe_eval', side_effect=eval) as unsafe_eval):
        ...
        with patch('thrive.tools.safe_eval.unsafe_eval', side_effect=eval) as unsafe_eval:
        ...
        for template, excepted in templates:
            ...
            with patch('thrive.addons.mail.models.mail_render_mixin.MailRenderMixin._render_template_qweb_regex', side_effect=o_render) as render:
                rendered = mail_template._render_field('body_html', record.ids)[record.id]
...
class TestMailTemplateReset(MailCommon):
    ...
    def test_mail_template_reset_translation(self):
        ...
        self.assertEqual(mail_template.body_html.strip(), Markup('<div>Hello Thrive</div>'))
        self.assertEqual(mail_template.with_context(lang='en_GB').body_html.strip(), Markup('<div>Hello Thrive</div>'))
...
class TestMailTemplateUI(HttpCase):
    ...
    def test_mail_template_dynamic_placeholder_tour(self):
        self.start_tour("/thrive", 'mail_template_dynamic_placeholder_tour', login="admin")
