...
from thrive.addons.mail.controllers.discuss.public_page import PublicPageController
...
class MailController(http.Controller):
    ...
    def _redirect_to_messaging(cls):
        url = '/thrive/action-mail.action_discuss'
    ...
    def _redirect_to_record(cls, model, res_id, access_token=None, **kwargs):
        ...
        if cids:
            request.future_response.set_cookie('cids', '-'.join([str(cid) for cid in cids]))
        url = f'/thrive/{model}/{res_id}?{url_encode(url_params)}'
        return request.redirect(url)
    ...
    def mail_thread_message_redirect(self, message_id, **kwargs):
        ...
        if message.model == 'discuss.channel':
            url = f'/thrive/action-mail.action_discuss?active_id={message.res_id}&highlight_message_id={message_id}'
        else:
            url = f'/thrive/{message.model}/{message.res_id}?highlight_message_id={message_id}'
        return request.redirect(url)
