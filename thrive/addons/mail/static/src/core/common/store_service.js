...
export class Store extends BaseStore {
    ...
    menuThreads = Record.many("Thread", {
        ...
        sort(a, b) {
             ...
             */
            const aThriveBot = a.isCorrespondentThriveBot;
            const bThriveBot = b.isCorrespondentThriveBot;
            if (aThriveBot && !bThriveBot) {
                return 1;
            }
            if (bThriveBot && !aThriveBot) {
                return -1;
            }
    ...
    async searchPartners(searchStr = "", limit = 10) {
        ...
        for (const localId in this.Persona.records) {
            ...
            if (
                ...
                ((partner.active && partner.userId) || partner === this.store.thrivebot)
            ) {
