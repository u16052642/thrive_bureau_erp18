...
import { Deferred, mockDate, tick } from "@thrive/hoot-mock";
import {
    Command,
    getService,
    makeKwArgs,
...
test("no out-of-focus notification on receiving self messages in chat", async () => {
    const pyEnv = await startServer();
    const channelId = pyEnv["discuss.channel"].create({ channel_type: "chat" });
    mockService("presence", { isThriveFocused: () => false });
    mockService("title", {
        setParts(parts) {
...
test("out-of-focus notif on needaction message in channel", async () => {
    ...
    // simulate receiving a new needaction message with thrive out-of-focused
...
test("receive new chat message: out of thrive focus (notification, chat)", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Dumbledore" });
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const channelId = pyEnv["discuss.channel"].create({
...
test("receive new chat messages: out of thrive focus (tab title)", async () => {
    let stepCount = 0;
    const pyEnv = await startServer();
...
test("Partner IM status is displayed as thread icon in top bar of channels of type 'chat'", async () => {
    ...
    pyEnv["discuss.channel"].create([
        ...
        },
        {
            channel_member_ids: [
                Command.create({ partner_id: serverState.partnerId }),
                Command.create({ partner_id: serverState.thrivebotId }),
            ],
            channel_type: "chat",
        },
    ]);
