...
import { Deferred, mockUserAgent } from "@thrive/hoot-mock";
import {
    Command,
    getService,
    makeKwArgs,
...
import { queryOne } from "@thrive/hoot-dom";
...
test("rendering with ThriveBot has a request (default)", async () => {
    patchBrowserNotification("default");
    const pyEnv = await startServer();
    const [thrivebot] = pyEnv["res.partner"].read(serverState.thrivebotId);
    await start();
    ...
    await contains(
        `.o-mail-NotificationItem img[data-src='${getOrigin()}/web/image/res.partner/${
            serverState.thrivebotId
        }/avatar_128?unique=${deserializeDateTime(thrivebot.write_date).ts}']`
    );
    await contains(".o-mail-NotificationItem", { text: "ThriveBot has a request" });
});

test("rendering without ThriveBot has a request (denied)", async () => {
    patchBrowserNotification("denied");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
...
test("rendering without ThriveBot has a request (accepted)", async () => {
...
test("no 'ThriveBot has a request' in mobile app", async () => {
    patchBrowserNotification("default");
    // simulate Android Thrive App
    mockUserAgent("Chrome/0.0.0 Android (ThriveMobile; Linux; Android 13; Thrive TestSuite)");
    await start();
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-MessagingMenu-counter", { count: 0 });
    await contains(".o-mail-NotificationItem", { count: 0 });
...
test("rendering with PWA installation request", async () => {
    ...
    await contains(".o-mail-NotificationItem-name", { text: "ThriveBot has a suggestion" });
    await contains(".o-mail-NotificationItem-text", {
        text: "Come here often? Install Thrive on your device!",
    });
...
test("installation of the PWA request can be dismissed", async () => {
    ...
    await assertSteps([
        ...
        'installationState value:  {"/thrive":"dismissed"}',
    ]);
    await click(".o_menu_systray i[aria-label='Messages']");
    await contains(".o-mail-NotificationItem", { count: 0 });
});
...
test("chat preview should not display correspondent name in body", async () => {
    ...
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Demo", email: "demo@thrivebureau.com" });
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const channelId = pyEnv["discuss.channel"].create({
        channel_type: "chat",
        channel_member_ids: [
...
test("click on preview should mark as read and open the thread", async () => {
    ...
    const messageId = pyEnv["mail.message"].create({
        model: "res.partner",
        body: "not empty",
        author_id: serverState.thrivebotId,
        needaction: true,
        res_id: partnerId,
    });
    pyEnv["mail.notification"].create({
