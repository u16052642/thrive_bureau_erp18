...
from thrive.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from thrive.tools.mail import single_email_re
from thrive.tools.misc import find_in_path, lower_logging
from thrive.tools.xml_utils import _validate_xml
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: thrive.tests.common.Form is deprecated, use thrive.tests.Form",
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    ...
    _tests_run_count = int(os.environ.get('THRIVE_TEST_FAILURE_RETRIES', 0)) + 1
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(thrive.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            ...
            popped_request = thrive.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('thrive.sql_db.Cursor.execute', execute),
    ...
    def assertRecordValues(
            self,
            records: thrive.models.BaseModel,
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | thrive.addons.base.models.res_currency.Currency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('thrive.addons.base.models.res_users.Users._crypt_context', _crypt_context)
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if thrive.service.server.server is None:
            return None
        return thrive.service.server.server.httpd.server_port
    ...
    def authenticate(self, user, password, browser: ChromeBrowser = None):
        ...
        if user: # if authenticated
            ...
            with patch('thrive.addons.base.models.res_users.Users._check_credentials', new=patched_check_credentials):
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', f"thrive.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"thrive.isTourReady({tour_name!r})")
...
class freeze_time:
    """ Object to replace the freezegun in Thrive test suites
