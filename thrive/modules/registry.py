...
from thrive.tools.misc import Collector, format_frame
...
class Registry(Mapping):
    ...
    def init(self, db_name):
        ...
        if config['test_enable'] or config['test_file']:
            from thrive.tests.result import ThriveTestResult  # noqa: PLC0415
            self._assertion_report = ThriveTestResult()
        else:
        ...
        self._db = thrive.sql_db.db_connect(db_name, readonly=False)
        ...
        if config['db_replica_host'] is not False or config['test_enable']:  # by default, only use readonly pool if we have a db_replica_host defined. Allows to have an empty replica host for testing
            self._db_readonly = thrive.sql_db.db_connect(db_name, readonly=True)
