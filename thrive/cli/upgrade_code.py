...
class UpgradeCode(Command):
    ...
    def run(self, cmdargs):
        thrive = Path(__file__).parent.parent.parent
        to_version = SourceFileLoader('release', str(next(thrive.glob('thrive/release.py')))).load_module().version_info
        from_version = (to_version[0] if to_version[1] else to_version[0] - 1,)
        addons_path = [
            str(next(thrive.glob('thrive/addons'))),
            str(next(thrive.glob('addons'))),
        ]

        parser = argparse.ArgumentParser()
        parser.add_argument('--addons-path', default=','.join(addons_path),
            help="[str] comma separated string representing the thrive addons path")
