...
def get_translated_module(arg: str | int | typing.Any) -> str:  # frame not represented as hint
    ...
    if isinstance(arg, str):
        if arg.startswith('thrive.addons.'):
            # get the name of the module
            return arg.split('.')[2]
        if '.' in arg or not arg:
            # module name is not in thrive.addons.
            return 'base'
        else:
            return arg
    else:
        ...
        if not frame:
            return 'base'
        if (module_name := frame.f_globals.get("__name__")) and module_name.startswith('thrive.addons.'):
...
def _get_cr(frame):
    ...
    if (local_self := frame.f_locals.get('self')) is not None:
        ...
        if (cr := getattr(local_self, 'cr', None)) is not None:
            return cr
    try:
        from thrive.http import request  # noqa: PLC0415
...
def get_po_paths(module_name: str, lang: str, env: thrive.api.Environment | None = None):
...
def _get_translation_upgrade_queries(cr, field):
    ...
    """
    from thrive.modules.registry import Registry  # noqa: PLC0415
